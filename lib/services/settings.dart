...
class ColorAdapter extends TypeAdapter<Color> {
  ...
  @override
  final fields = const {0: ColorFields.value};
...
enum GallerySavePathOrganizing {
  ...
  siteSubfolders,
  @HiveField(7)
  siteAndBoardSubfolders,
  @HiveField(8)
  siteBoardAndThreadSubfolders,
  @HiveField(9)
...
void _readHookSavedSettingsFields(List<dynamic> fields) {
  ...
  fields[SavedSettingsFields.kWatchThreadAutomaticallyWhenCreating] ??=
      fields[SavedSettingsFields.kWatchThreadAutomaticallyWhenReplying] ?? true;
}
...
class SavedSettings extends HiveObject {
  ...
  SavedSettings({
    ...
    if (getInappropriateUserAgents().contains(userAgent) &&
        !getAppropriateUserAgents().contains(userAgent)) {
    ...
    // Migrate to gallery://
    if (Platform.isIOS &&
        gallerySavePathOrganizing != null &&
        gallerySavePath == null) {
      if (gallerySavePathOrganizing ==
          GallerySavePathOrganizing.deprecatedNoFolder) {
  ...
  TextStyle get textStyle {
    ...
    if (fontFamily == null) {
      return platformIsMaterial
          ? const TextStyle(fontFamily: 'Roboto', fontSize: 16)
          : const TextStyle(fontFamily: 'CupertinoSystemText', fontSize: 16);
    }
...
abstract class MutableSetting<T> {
  ...
  T get(BuildContext context, bool listen) =>
      listen ? watch(context) : read(context);
  Future<void> didMutate(BuildContext context);
  Future<void> Function() makeDidMutate(BuildContext context) =>
      () => didMutate(context);
...
class MutableSavedSetting<T> extends MutableSetting<T> {
  ...
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is MutableSavedSetting && other.setting == setting;
...
class DummyMutableSetting<T> extends MutableSetting<T> {
  ...
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is DummyMutableSetting && other.value == value;
  @override
...
class SavedSetting<T> extends ImmutableSetting<T> {
  ...
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is SavedSetting && other.setting == setting;
...
class SavedSettingEquals<T> extends MutableSetting<bool> {
  ...
  @override
  bool read(BuildContext context) =>
      setting.getter(Settings.instance.settings) == value;
...
class MappedSetting<T, New> extends ImmutableSetting<New> {
  ...
  @override
  Future<void> write(BuildContext context, New value) =>
      setting.write(context, reverse(value));
...
class CombinedSetting<T1, T2> extends ImmutableSetting<(T1, T2)> {
  ...
  @override
  (T1, T2) read(BuildContext context) =>
      (setting1.read(context), setting2.read(context));

  @override
  (T1, T2) watch(BuildContext context) =>
...
class CombinedMutableSetting<T1, T2> extends MutableSetting<(T1, T2)> {
  ...
  @override
  Future<void> didMutate(BuildContext context) =>
      Future.wait([setting1.didMutate(context), setting2.didMutate(context)]);
...
class HookedSetting<T> extends ImmutableSetting<T> {
  ...
  Future<void> write(BuildContext context, T value) async {
    if ((await beforeChange?.call(context, read(context), value) ?? true) &&
        context.mounted) {
...
class AssociatedSetting<Old, Associated, New> extends ImmutableSetting<New> {
  ...
  New read(BuildContext context) =>
      forwards(setting.read(context), associated.read(context));

  @override
  New watch(BuildContext context) =>
      forwards(setting.watch(context), associated.watch(context));

  @override
  Future<void> write(BuildContext context, New value) =>
      setting.write(context, reverse(value, associated.read(context)));

  @override
  List<String> get syncPaths => [...setting.syncPaths, ...associated.syncPaths];
...
class Settings extends ChangeNotifier {
  ...
  bool get isConnectedToWifiForCookies =>
      !separateWiFiAndCellularCookies || isConnectedToWifi;
  ...
  bool get autoloadAttachments {
    return (_settings.autoloadAttachments ==
        ...
        ((_settings.autoloadAttachments == AutoloadAttachmentsSetting.wifi) &&
            isConnectedToWifi);
  }
  ...
  bool showImages(BuildContext context, String board) {
    return _settings.contentSettings.images &&
        ((context.read<Persistence>().maybeGetBoard(board)?.isWorksafe ??
                false) ||
            _settings.contentSettings.nsfwImages);
  }
  ...
  bool get boardSwitcherHasKeyboardFocus =>
      _settings.boardSwitcherHasKeyboardFocus;
  ...
  SavedTheme get lightTheme =>
      _settings.themes[_settings.lightThemeKey] ?? defaultLightTheme;
  SavedTheme get darkTheme =>
      _settings.themes[_settings.darkThemeKey] ?? defaultDarkTheme;
  String get themeKey =>
      whichTheme == Brightness.dark ? darkThemeKey : lightThemeKey;
  SavedTheme get theme =>
      whichTheme == Brightness.dark ? darkTheme : lightTheme;
  ...
  bool get fullQualityThumbnails {
    return (fullQualityThumbnailsSetting ==
            AutoloadAttachmentsSetting.always) ||
        ((fullQualityThumbnailsSetting == AutoloadAttachmentsSetting.wifi) &&
            isConnectedToWifi);
  }
  ...
  bool get autoCacheAttachments {
    return (autoCacheAttachmentsSetting == AutoloadAttachmentsSetting.always) ||
        ((autoCacheAttachmentsSetting == AutoloadAttachmentsSetting.wifi) &&
            isConnectedToWifi);
  }
  ...
  bool get loadThumbnails {
    return (loadThumbnailsSetting == AutoloadAttachmentsSetting.always) ||
        ((loadThumbnailsSetting == AutoloadAttachmentsSetting.wifi) &&
            isConnectedToWifi);
  }
  ...
  set androidLegacyStatusBarsEnabled(bool setting) =>
      setLegacyStatusBarsEnabled(setting).then((_) {
        notifyListeners();
      });
  ...
  void runQuickAction(BuildContext context) {
    ...
    switch (settingsQuickAction) {
      ...
      case SettingsQuickAction.toggleInterfaceStyle:
        ...
        supportMouseSetting.value = mouseSettings.supportMouse
            ? TristateSystemSetting.a
            : TristateSystemSetting.b;
        showToast(
            context: context,
...
class _SettingsSystemListenerState extends State<SettingsSystemListener>
  ...
  void didChangePlatformBrightness() {
    Settings.instance.systemBrightness =
