...
Future<KurobaImportResult> importKurobaZip(String zipPath) async {
  try {
    ...
    if (imageboard == null) {
      return KurobaImportFailure(
          'No imageboard configured for site: $siteName');
    }
    ...
    final htmlContent =
        htmlFile.existsSync() ? htmlFile.readAsStringSync() : null;
    ...
    try {
      ...
      if (htmlContent == null) {
        return KurobaImportFailure(
            'Thread not found online and no thread_data.html in ZIP.');
      }
      final htmlThread =
          parseKurobaThreadHtml(imageboard.key, board, threadId, htmlContent);
      if (htmlThread == null) {
        return KurobaImportFailure(
            'Failed to parse any posts from thread_data.html');
      }
      thread = htmlThread;
...
Thread? parseKurobaThreadHtml(
  ...
  for (final container in postContainers) {
    ...
    posts.add(Post(
      ...
      threadId: threadId,
      id: postId,
      spanFormat: imageboard.site is Site4Chan
          ? PostSpanFormat.chan4
          : PostSpanFormat.stub,
  ...
  opTime ??= DateTime.now();
  final imageCount =
      posts.fold<int>(0, (sum, p) => sum + p.attachments_.length);
  return Thread(
    posts_: posts,
    isArchived: true,
