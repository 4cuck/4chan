...
class SliverStaggeredGridDelegate extends SliverGridDelegate {
  ...
  SliverGridLayout getLayout(SliverConstraints constraints) {
    ...
    if (aspectRatios.length > columnCount) {
      for (int swap = 0; swap < columnCount; swap++) {
        ...
        if (columns[tallestColumn].length > 1) {
          final toMove =
              columns[tallestColumn][columns[tallestColumn].length - 2];
    ...
    return SliverStaggeredGridLayout(columns: columns, columnWidth: width);
  }

  @override
  bool shouldRelayout(SliverStaggeredGridDelegate oldDelegate) {
    return !listEquals(aspectRatios, oldDelegate.aspectRatios) ||
...
class SliverStaggeredGridLayout extends SliverGridLayout {
  ...
  int getMaxChildIndexForScrollOffset(double scrollOffset) {
    return _lookupTable.values
        .where((w) => w.$2.offset < scrollOffset)
        .fold<int>(0, (t, x) => max(t, x.$2.index));
  }

  @override
  int getMinChildIndexForScrollOffset(double scrollOffset) {
    return _lookupTable.values
        .where((w) => (w.$2.height + w.$2.offset) >= scrollOffset)
        .fold<int>(_lookupTable.length - 1, (t, x) => min(t, x.$2.index));
  }
}
