...
bool _getDontSupportMouse(BuildContext context) =>
    !context.read<MouseSettings>().supportMouse;
bool _watchDontSupportMouse(BuildContext context) =>
    !context.select<MouseSettings, bool>((s) => s.supportMouse);
...
extension _UseTree on Imageboard {
  ImmutableSetting<bool> get useTree => SettingWithFallback(
      SavedSetting(ChainedFieldWriter(
          ChainedFieldReader(SavedSettingsFields.browserStateBySite,
              MapFieldWriter<String, PersistentBrowserState>(key: key)),
          PersistentBrowserStateFields.useTree)),
      site.useTree);
  ImmutableSetting<bool> get dontUseTree =>
      MappedSetting(useTree, FieldMappers.invert, FieldMappers.invert);
}
  ...
  return Thread(
      ...
      posts_: [
        ...
        Post(
            board: 'tv',
            text:
                '<a href="#p123455" class="quotelink">&gt;&gt;22568140</a>\nThis is the first reply to the OP.',
            name: 'User',
            trip: '!fdsa',
...
Widget _buildFakeThreadRow(ThreadRowStyle style) => Builder(builder: (context) {
      final theme = context.read<SavedTheme>();
      return HeroMode(
          enabled: false,
          child: Container(
              decoration: style.isGrid
                  ? const BoxDecoration()
                  : BoxDecoration(
                      border: Border(
                          top: BorderSide(
                              color: theme.primaryColorWithBrightness(20)),
                          bottom: BorderSide(
                              color: theme.primaryColorWithBrightness(20)))),
              padding: style.isGrid
                  ? EdgeInsets.zero
                  : const EdgeInsets.symmetric(vertical: 1),
              child: ThreadRow(
                  style: style, isSelected: false, thread: _makeFakeThread())));
    });
...
Widget _buildFakePostRow() {
  ...
  return ChangeNotifierProvider<PostSpanZoneData>(
      ...
      child: PostRow(isSelected: false, post: thread.posts.first));
}
...
Future<void> _cleanupFontFile(String? family) async {
  if (family == Settings.instance.fontFamily ||
      family == Settings.instance.fontFamilyFallback) {
    // Still in use
    return;
  }
  if (family != null && (family.endsWith('.ttf') || family.endsWith('.otf'))) {
    ...
    try {
      await Persistence.documentsDirectory
          .dir(Persistence.fontsDir)
          .file(family)
          .delete();
    } catch (e, st) {
      Future.error(e, st);
...
final appearanceSettings = [
  ...
  SettingHiding(
      hidden: MappedMutableSetting(
          Settings.androidDrawerSetting, FieldMappers.invert),
      setting: const SwitchSettingWidget(
          description: 'Drawer permanently visible',
          icon: CupertinoIcons.sidebar_left,
          helpText:
              'The drawer will always be on the left side if there is enough space. On devices with a hinge, the drawer will size itself to fill the left screen.',
  ...
  SettingHiding(
      ...
      setting: const SwitchSettingWidget(
          ...
          icon: CupertinoIcons.sidebar_left,
          helpText:
              'The drawer can be opened by swiping right from anywhere on the left half of the screen, even when a thread is open.',
  ...
  for (final fontSetting in [
    ...
    (
      ...
      hidden: const MappedMutableSetting(
          Settings.fontFamilySetting, FieldMappers.isNull)
    )
  ])
    SettingHiding(
        ...
        setting: ImmutableButtonSettingWidget(
            ...
            setting: CustomImmutableSetting(
                ...
                writer: (context, newFont) async {
                  if (newFont == null) {
                    ...
                    fontSetting.setting.setting
                        .setter(Persistence.settings, newFont);
                    // ignore: invalid_use_of_protected_member, invalid_use_of_visible_for_testing_member
                    Settings.instance.notifyListeners();
                    // Allow some time for UI to rerender and maybe crash
                    Future.delayed(
                        const Duration(seconds: 5), Settings.instance.didEdit);
                  }
                }),
            injectButton: (fontSetting.error == null)
                ? null
                : (context, fontFamily, setFontFamily) => AdaptiveIconButton(
            ...
            onPressed: (context, family, setFamily) async {
              final availableFonts = await showAdaptiveDialog<List<String>>(
                  ...
                  context: context,
                  builder: (context) => AdaptiveAlertDialog(
                          title: const Text('Choose a font source',
                          ...
                          actions: [
                            AdaptiveDialogAction(
                                ...
                                onPressed: () async {
                                  try {
                                    final fonts =
                                        await getInstalledFontFamilies(context);
                                    if (context.mounted) {
                            ...
                            AdaptiveDialogAction(
                                ...
                                onPressed: () => Navigator.pop(
                                    context, allowedGoogleFonts.keys.toList())),
                            AdaptiveDialogAction(
                                ...
                                onPressed: () async {
                                  try {
                                    ...
                                    final path =
                                        pickerResult?.files.tryFirst?.path;
                                    if (path == null) {
                                      return;
                                    }
                                    final basename = FileBasename.get(path);
                                    final ttfFolder = await Persistence
                                        .documentsDirectory
                                        .dir(Persistence.fontsDir)
                                        .create();
                                    await File(path)
                                        .copy(ttfFolder.child(basename));
                                    if (context.mounted) {
              ...
              final selectedFont = availableFonts.trySingle ??
                                  ...
                                  child: CupertinoScrollbar(
                                      child: ListView.separated(
                                          ...
                                          itemBuilder: (context, i) => AdaptiveDialogAction(
                                              onPressed: () => Navigator.pop(
                                                  context, availableFonts[i]),
                                              child: Text(availableFonts[i],
                                                  style: allowedGoogleFonts[availableFonts[i]]
                                                          ?.call() ??
                                                      TextStyle(
  ...
  for (final theme in [
    ...
    CustomMutableSettingWidget<SavedTheme>(
        ...
        setting: CustomMutableSetting(
            reader: (context) =>
                Settings.instance.themes[theme.setting.value] ??
                Settings.instance.lightTheme,
            watcher: (context) {
              ...
              return Settings.instance.themes[key] ??
                  Settings.instance.lightTheme;
            },
            ...
            child: SingleChildScrollView(
                ...
                child: Row(
                  ...
                  (
                    'Primary',
                    theme.primaryColor,
                    (c) => theme.primaryColor = c,
                    theme.copiedFrom?.primaryColor
                  ),
                  (
                    'Secondary',
                    theme.secondaryColor,
                    (c) => theme.secondaryColor = c,
                    theme.copiedFrom?.secondaryColor
                  ),
                  (
                    'Bar',
                    theme.barColor,
                    (c) => theme.barColor = c,
                    theme.copiedFrom?.barColor
                  ),
                  (
                    'Background',
                    theme.backgroundColor,
                    (c) => theme.backgroundColor = c,
                    theme.copiedFrom?.backgroundColor
                  ),
                  (
                    'Quote',
                    theme.quoteColor,
                    (c) => theme.quoteColor = c,
                    theme.copiedFrom?.quoteColor
                  ),
                  (
                    'Title',
                    theme.titleColor,
                    (c) => theme.titleColor = c,
                    theme.copiedFrom?.titleColor
                  ),
                  (
                    'Link',
                    theme.linkColor,
                    (c) => theme.linkColor = c,
                    theme.copiedFrom?.linkColor
                  ),
                  (
                    'Text Field',
                    theme.textFieldColor,
                    (c) => theme.textFieldColor = c,
                    theme.copiedFrom?.textFieldColor
                  )
                ]
                        .map(
                            (color) => Column(
                                          ...
                                          builder: (context) => CupertinoButton(
                                              ...
                                              onPressed: () async {
                                                if (theme.locked) {
                                                  alertError(
                                                      context,
                                                      'This theme is locked. Make a copy of it if you want to change its colours.',
                                                      null);
                                                  return;
                                                }
                                                Color c = color.$2;
                                                                  ...
                                                                  child: Padding(
                                                                      ...
                                                                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                                                                        ...
                                                                        AdaptiveFilledButton(
                                                                            ...
                                                                            color: color.$4,
                                                                            onPressed: (c == color.$4 || color.$4 == null)
                                                                                ? null
                                                                                : () {
                                                                                    c = color.$4!;
                                                                                    didModify();
  ...
  SliderSettingWidget(
      ...
      enabledSetting: MappedSetting(
          ...
          (size) => size > 0,
          (enabled) =>
              Settings.instance.centeredPostThumbnailSizeSetting.abs() *
              (enabled ? 1 : -1))),
  SliderSettingWidget(
      ...
      widgetFormatter: (brightness) => Builder(
          builder: (context) => Container(
              color: context.select<SavedTheme, Color>(
                  (t) => t.primaryColorWithBrightness(brightness)),
  ...
  const SegmentedSettingWidget(
      ...
      children: {false: (null, 'Left'), true: (null, 'Right')},
  ...
  SliderSettingWidget(
      ...
      setting: const MappedSetting(Settings.thumbnailPixelationSetting,
          FieldMappers.toDoubleAbs, FieldMappers.toIntAbs),
      min: 3,
      max: 75,
      step: 1,
      textFormatter: (s) => 'Size: ${s.abs().round()}x${s.abs().round()}',
      enabledSetting: MappedSetting(
          ...
          (enabled) =>
              Settings.instance.thumbnailPixelation.abs() *
              (enabled ? 1 : -1))),
  SliderSettingWidget(
      description: 'Dim image thumbnails',
      icon: CupertinoIcons.square_righthalf_fill,
      setting: const MappedSetting(Settings.thumbnailOpacitySetting,
          FieldMappers.doubleAbs, FieldMappers.doubleAbs),
      min: 0,
      max: 1,
      step: 0.01,
      ...
      enabledSetting: MappedSetting(
          ...
          (opacity) => opacity > 0,
          (enabled) =>
              Settings.instance.thumbnailOpacity.abs() * (enabled ? 1 : -1))),
  const SwitchSettingWidget(
  ...
  PopupSubpageSettingWidget(
      ...
      preview: PanelSettingWidget(
          builder: (context) => Container(
              ...
              child: IgnorePointer(child: _buildFakePostRow()))),
      settings: [
        const SwitchSettingWidget(
        ...
        const SwitchSettingWidget(
            description: 'Show name', setting: Settings.showNameOnPostsSetting),
        const SwitchSettingWidget(
            description: 'Hide default names',
            setting: Settings.hideDefaultNamesOnPostsSetting),
        const SwitchSettingWidget(
            description: 'Show trip', setting: Settings.showTripOnPostsSetting),
        const SwitchSettingWidget(
        ...
        const SwitchSettingWidget(
            description: 'Show pass', setting: Settings.showPassOnPostsSetting),
        const SwitchSettingWidget(
            description: 'Show flag', setting: Settings.showFlagOnPostsSetting),
        const SwitchSettingWidget(
                                  ...
                                  children: list.asMap().entries.map((pair) {
                                    ...
                                    switch (pair.value) {
                                      case PostDisplayField.name:
                                        disabled = !settings.showNameOnPosts &&
                                            !settings.showTripOnPosts;
                                        break;
                                      case PostDisplayField.attachmentInfo:
                                        disabled = !settings
                                                .showFilenameOnPosts &&
                                            !settings.showFilesizeOnPosts &&
                                            !settings.showFileDimensionsOnPosts;
                                        break;
                                      ...
                                      case PostDisplayField.countryName:
                                        disabled =
                                            !settings.showCountryNameOnPosts;
                                        break;
                                      case PostDisplayField.absoluteTime:
                                        disabled =
                                            !settings.showAbsoluteTimeOnPosts;
                                        break;
                                      case PostDisplayField.relativeTime:
                                        disabled =
                                            !settings.showRelativeTimeOnPosts;
                                        break;
                                      case PostDisplayField.ipNumber:
                                        disabled =
                                            !settings.showIPNumberOnPosts;
                                        break;
                                      case PostDisplayField.postNumber:
                                        disabled =
                                            !settings.showPostNumberOnPosts;
                                        break;
                                      case PostDisplayField.lineBreak1:
                                        disabled = !settings
                                            .showLineBreak1InPostInfoRow;
                                        break;
                                      case PostDisplayField.lineBreak2:
                                        disabled = !settings
                                            .showLineBreak2InPostInfoRow;
                                        break;
                                    ...
                                    return ReorderableDelayedDragStartListener(
                                        ...
                                        child: Container(
                                            ...
                                            child: Text(pair.value.displayName,
                                                style: disabled
                                                    ? TextStyle(
                                                        color:
                                                            ChanceTheme.primaryColorWithBrightness50Of(
                                                                context))
                                                    : null)));
                                  }).toList(),
  ...
  SegmentedSettingWidget(
      ...
      injectButton: (context, useGrid, setUseGrid) => (ImageboardRegistry
                  .instance.count >
              1)
          ? Builder(
                  ...
                  onPressed: () => Navigator.push(
                      ...
                      adaptivePageRoute(
                          builder: (context) => SettingListPage(
                              ...
                              settings: ImageboardRegistry.instance.imageboards
                                  .map((imageboard) => SegmentedSettingWidget(
                                          ...
                                          setting: MappedSetting(
                                              SavedSetting(ChainedFieldWriter(
                                                  ChainedFieldReader(
                                                    SavedSettingsFields
                                                        .browserStateBySite,
                                                    MapFieldWriter<String,
                                                            PersistentBrowserState>(
                                                        key: imageboard.key),
                                                  ),
                                                  PersistentBrowserStateFields
                                                      .useCatalogGrid)),
                                              FieldMappers.nullSafeOptionalify,
                                              FieldMappers
                                                  .unNullSafeOptionalify),
                                          children: {
                                            NullSafeOptional.false_: (
                                              CupertinoIcons.rectangle_grid_1x2,
                                              'Rows'
                                            ),
                                            NullSafeOptional.null_: (
                                              null,
                                              'Default (${useGrid ? 'Grid' : 'Rows'})'
                                            ),
                                            NullSafeOptional.true_: (
                                              CupertinoIcons
                                                  .rectangle_split_3x3,
                                              'Grid'
                                            ),
      ...
      preview: PanelSettingWidget(
          builder: (context) => SizedBox(
              ...
              child: Stack(children: [
                if (Settings.maxCatalogRowHeightSetting.watch(context) != null)
                  Positioned.fill(
                      child: Container(
                          decoration: BoxDecoration(
                              border: Border.all(
                ...
                DecoratedBox(
                    ...
                    decoration: BoxDecoration(
                        border: Border(
                      top: BorderSide(
                          color: ChanceTheme.primaryColorWithBrightness20Of(
                              context)),
                      bottom: BorderSide(
                          color: ChanceTheme.primaryColorWithBrightness20Of(
                              context)),
                    ...
                    child: ThreadRow(
                        ...
                        showLastReplies: Settings
                            .showLastRepliesInCatalogSetting
                            .watch(context)))
              ]))),
      settings: [
        const SwitchSettingWidget(
  ...
  PopupSubpageSettingWidget(
        ...
        if (staggered) {
          return Stack(children: [
            Container(
                ...
                decoration: BoxDecoration(
                    border:
                        Border.all(color: ChanceTheme.primaryColorOf(context))),
                alignment: Alignment.bottomCenter,
                child: const Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
      ...
      settings: [
        ...
        SliderSettingWidget(
            ...
            step: 1,
            textFormatter: (width) =>
                '${width.round()} px (${describeCount(_estimateGridModeColumns(width), 'column')})'),
        SliderSettingWidget(
            description: 'Height',
            keywords: ['size'],
  ...
  SliderSettingWidget(
      ...
      step: 50,
      max: 3000,
      textFormatter: (twoPaneBreakpoint) =>
          '${twoPaneBreakpoint.round()} pixels',
      helpText:
          'When the screen is at least ${Settings.instance.twoPaneBreakpoint.round()} pixels wide, two columns will be used.\nThe board catalog will be on the left and the current thread will be on the right.'),
  SliderSettingWidget(
      description: 'Two-pane split',
      icon: CupertinoIcons.sidebar_left,
      setting: const MappedSetting(Settings.twoPaneSplitSetting,
  ...
  SliderSettingWidget(
      ...
      enabledSetting: MappedSetting(
          ...
          (x) => x >= 0,
          (enabled) =>
              Settings.instance.verticalTwoPaneMinimumPaneSize.abs() *
              (enabled ? 1 : -1)),
      min: 100,
      max: 1000,
      step: 25,
      textFormatter: (verticalTwoPaneMinimumPaneSize) =>
          'Minimum pane height: ${verticalTwoPaneMinimumPaneSize.abs().round()} px'),
  const SwitchSettingWidget(
  ...
  SegmentedSettingWidget(
      ...
      setting: MappedSetting<(bool, bool), NullSafeOptional>(
          const CombinedSavedSetting(
              Settings.showScrollbarsSetting, Settings.scrollbarsOnLeftSetting),
  ...
  SegmentedSettingWidget(
          ...
          builder: (context) => Row(mainAxisSize: MainAxisSize.min, children: [
                Container(
                    decoration: BoxDecoration(
                        ...
                        color: ChanceTheme.primaryColorWithBrightness60Of(
                            context)),
      ...
      children: const {true: (null, 'Left'), false: (null, 'Right')}),
  if (Platform.isAndroid && Settings.featureStatusBarWorkaround)
    const SwitchSettingWidget(
        description: 'Use status bar workaround',
        icon: CupertinoIcons.device_phone_portrait,
        helpText:
            'Some devices have a bug in their Android ROM, where the status bar cannot be properly hidden.\n\nIf this workaround is enabled, the status bar will not be hidden when opening the gallery.',
        setting:
            SettingWithFallback(Settings.useStatusBarWorkaroundSetting, false)),
  if (Platform.isAndroid)
    SwitchSettingWidget(
        description: 'Alternative status bar transparency',
        icon: Icons.cell_wifi,
        helpText:
            'Enable this setting to use an alternative approach to set the status bar to transparent. This is known to be useful on Xiaomi phones.',
        setting: CustomImmutableSetting(
            reader: (context) =>
                context.read<Settings>().androidLegacyStatusBarsEnabled,
            watcher: (context) =>
                context.watch<Settings>().androidLegacyStatusBarsEnabled,
            writer: (context, setting) async {
              context.read<Settings>().androidLegacyStatusBarsEnabled = setting;
            })),
  ImageboardScopedSettingGroup(title: 'Thread layout', settings: [
    ...
    ImageboardScopedSettingWidget(
        ...
        builder: (threadLayoutImageboard) => SwitchSettingWidget(
            ...
            setting: SavedSetting(ChainedFieldWriter(
                ChainedFieldReader(
                    SavedSettingsFields.browserStateBySite,
                    MapFieldWriter<String, PersistentBrowserState>(
                        key: threadLayoutImageboard.key)),
                PersistentBrowserStateFields
    ...
    ImageboardScopedSettingWidget(
        ...
        builder: (threadLayoutImageboard) => SwitchSettingWidget(
            ...
            helpText:
                'New posts will be added below the main tree to make it easier to read them. Pulling to refresh will re-sort them into the main tree.',
            setting: SavedSetting(ChainedFieldWriter(
                ChainedFieldReader(
                    SavedSettingsFields.browserStateBySite,
                    MapFieldWriter<String, PersistentBrowserState>(
  ...
  SegmentedSettingWidget<NullWrapper<String>>(
      ...
      setting: MappedSetting<(bool, String), NullWrapper<String>>(
          ...
          (mapped) => switch (mapped.value) {
                null => (
                    false,
                    DateTimeConversion.kISO8601DateFormat /* arbitrary */
                  ),
                String format => (true, format)
              }),
      children: {
        const NullWrapper(null): (
          null,
          'Default (${DateTime.now().weekdayShortName})'
        ),
        const NullWrapper(DateTimeConversion.kISO8601DateFormat): (
          null,
          'ISO 8601 (${DateTime.now().formatDate(DateTimeConversion.kISO8601DateFormat)})'
        ),
        const NullWrapper('MM/DD/YY'): (
          null,
          'MM/DD/YY (${DateTime.now().formatDate('MM/DD/YY')})'
        ),
        const NullWrapper('DD/MM/YY'): (
          null,
          'DD/MM/YY (${DateTime.now().formatDate('DD/MM/YY')})'
        )
      }),
  const SwitchSettingWidget(
  ...
  const SwitchSettingWidget(
      description: 'Use quilt gallery layout',
      keywords: ['gallery', 'grid', 'quilt', 'media'],
      icon: CupertinoIcons.rectangle_grid_2x2,
      setting: Settings.useAlternativeGalleryLayoutSetting),
  const SwitchSettingWidget(
  ...
  const SegmentedSettingWidget(
      ...
      children: {false: (null, 'Continuous grid'), true: (null, 'Paged')},
  ...
  SteppableSettingWidget(
      ...
      setting: AssociatedSetting<double, double, int>(
          ...
          forwards: (maxExtent, width) =>
              (width / math.max(1, maxExtent)).ceil(),
