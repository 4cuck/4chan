...
class SettingsCachePanel extends StatefulWidget {
  const SettingsCachePanel({Key? key}) : super(key: key);
            ...
            children: [
              ...
              SizedBox(
                  ...
                  child: SingleChildScrollView(
                    ...
                    else if (folderSizes?.isEmpty ?? true)
                      const Text('No cached media'),
                    Table(
                        columnWidths: const {
                          0: FlexColumnWidth(),
              ...
              Wrap(
                  ...
                  children: [
                    ...
                    AdaptiveFilledButton(
                        padding: const EdgeInsets.all(16),
                        onPressed: (folderSizes?.isEmpty ?? true)
                            ? null
                            : (clearing ? null : _clearCaches),
...
class SettingsThreadsPanel extends StatelessWidget {
  const SettingsThreadsPanel({Key? key}) : super(key: key);
        ...
        builder: (context, child) {
          ...
          final oldThreadRows = [0, 7, 14, 30, 60, 90, 180].map((days) {
            ...
            final oldThreads = threadStateBox.values.where((state) {
              return (state.savedTime == null) &&
                  (state.threadWatch == null) &&
                  state.lastOpenedTime.compareTo(cutoff).isNegative;
            }).toList();
            return (days, oldThreads);
          }).toList();
          oldThreadRows.removeRange(
              oldThreadRows.lastIndexWhere((r) => r.$2.isNotEmpty) + 1,
              oldThreadRows.length);
          confirmDelete(List<PersistentThreadState> toDelete,
              {String itemType = 'thread'}) async {
            final confirmed = await showAdaptiveDialog<bool>(
                context: context,
                builder: (context) => AdaptiveAlertDialog(
                        title: const Text('Confirm deletion'),
                        content: Text(
                            '${describeCount(toDelete.length, itemType)} will be deleted'),
                        actions: [
          ...
          final savedThreads =
              threadStateBox.values.where((t) => t.savedTime != null).toList();
          final watchedThreads = threadStateBox.values
              .where((i) => i.threadWatch != null)
              .toList();
          return Container(
            decoration: BoxDecoration(
                borderRadius: const BorderRadius.all(Radius.circular(8)),
                color:
                    ChanceTheme.primaryColorOf(context).withValues(alpha: 0.2)),
            padding: const EdgeInsets.all(16),
            child: Table(
                columnWidths: const {
                  0: FlexColumnWidth(1),
                ...
                children: [
                  TableRow(children: [
                    ...
                    Padding(
                        ...
                        child: Text(savedThreads.length.toString(),
                            textAlign: TextAlign.right)),
                    AdaptiveIconButton(
                        onPressed: savedThreads.isEmpty
                            ? null
                            : () => confirmDelete(savedThreads,
                                itemType: 'saved thread'),
                  ...
                  TableRow(children: [
                    ...
                    Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 16),
                        child: Text(watchedThreads.length.toString(),
                            textAlign: TextAlign.right)),
                    AdaptiveIconButton(
                        onPressed: watchedThreads.isEmpty
                            ? null
                            : () => confirmDelete(watchedThreads,
                                itemType: 'watched thread'),
                        icon: const Text('Delete'))
                  ]),
                  ...oldThreadRows.map((entry) {
                    return TableRow(children: [
                      Text('Over ${entry.$1} days old',
                          textAlign: TextAlign.left),
                      Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 16),
                          child: Text(entry.$2.length.toString(),
                              textAlign: TextAlign.right)),
                      AdaptiveIconButton(
                          onPressed: entry.$2.isEmpty
                              ? null
                              : () => confirmDelete(entry.$2),
                          icon: const Text('Delete'))
                    ]);
                  })
                ]),
...
class ImportLogSummaryWidget extends StatelessWidget {
  ...
  const ImportLogSummaryWidget({required this.summary, super.key});
...
class ImportLogFailureWidget extends StatelessWidget {
  ...
  const ImportLogFailureWidget({required this.failure, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('${failure.type} Failure',
              style: TextStyle(fontSize: 16, color: color)),
          Row(children: [
            Icon(CupertinoIcons.doc, size: 14, color: color),
            const SizedBox(width: 4),
            Expanded(
                child: Text(failure.filename,
...
class ImportLogConflictWidget<Ancestor, T> extends StatefulWidget {
  ...
  const ImportLogConflictWidget({required this.conflict, super.key});

  @override
  createState() => _ImportLogConflictWidgetState();
}

class _ImportLogConflictWidgetState<Ancestor, T>
    extends State<ImportLogConflictWidget<Ancestor, T>> {
  bool accepted = false;
  ...
  Widget build(BuildContext context) {
    return Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('${widget.conflict.type} Conflict',
              ...
              child: Row(
                ...
                Flexible(
                    ...
                    child: AdaptiveThinButton(
                        onPressed: accepted
                            ? () {
                                widget.conflict.conflict
                                    .set(widget.conflict.yours, originalYours);
                                // Hack
                ...
                Flexible(
                    ...
                    child: AdaptiveThinButton(
                        onPressed: accepted
                            ? null
                            : () {
                                widget.conflict.conflict.set(
                                    widget.conflict.yours,
                                    widget.conflict.conflict
                                        .get(widget.conflict.theirs));
                                // Hack
                                if (widget.conflict.yours is SavedSettings) {
                                  Settings.instance.didEdit();
                                }
                        ...
                        child: Column(
                            ...
                            children: [
                              const Text('Imported',
                                  style: TextStyle(fontSize: 15)),
                              Text.rich(_importFieldValue(widget
                                  .conflict.conflict
                                  .get(widget.conflict.theirs)))
                            ])))
              ]))
...
class _CopyPartyMigrationDialog extends StatefulWidget {
  ...
  @override
  State<_CopyPartyMigrationDialog> createState() =>
      _CopyPartyMigrationDialogState();
}
...
final dataSettings = [
          ...
          beforeChange: (context, oldValue, newValue) async {
            try {
              final result = await LocalAuthentication().authenticate(
                  localizedReason: 'Verify access to app',
  ...
  ImmutableButtonSettingWidget(
      ...
      builder: (gallerySavePath) =>
          Text(gallerySavePath == null ? 'Set' : 'Change'),
      onPressed: (context, currentPath, setPath) async {
        ...
        if (newPath != null) {
          if (newPath.startsWith(kGallerySavePathGalleryPrefix)) {
            final newSavePathOrganizing = switch (
                Settings.gallerySavePathOrganizingSetting.read(context)) {
              GallerySavePathOrganizing.boardAndThreadSubfolders ||
              GallerySavePathOrganizing.boardAndThreadNameSubfolders =>
                (GallerySavePathOrganizing.boardSubfolders, 'Per-board'),
              GallerySavePathOrganizing.threadNameSubfolders => (
                  GallerySavePathOrganizing.noSubfolders,
                  'One album'
                ),
              GallerySavePathOrganizing.siteBoardAndThreadSubfolders ||
              GallerySavePathOrganizing.siteBoardAndThreadNameSubfolders ||
              GallerySavePathOrganizing.siteAndThreadNameSubfolders =>
                (GallerySavePathOrganizing.siteSubfolders, 'Per-site'),
              _ => null
            };
            if (newSavePathOrganizing != null) {
              final reallyChange = await confirm(context, 'Warning',
                  content:
                      'Your current save path organizing structure is not compatible with gallery saving. It will be changed to ${newSavePathOrganizing.$2}');
              if (!reallyChange || !context.mounted) {
                return;
              }
              Settings.gallerySavePathOrganizingSetting
                  .write(context, newSavePathOrganizing.$1);
            }
          }
          setPath(newPath);
        }
  ...
  if (Platform.isAndroid)
    ImmutableButtonSettingWidget(
        ...
        builder: (androidGalleryPicker) =>
            Text(androidGalleryPicker == null ? 'Set' : 'Change'),
  ...
  const SegmentedSettingWidget(
      ...
      children: {false: (null, 'User-submitted'), true: (null, 'Server-side')}),
  SettingBuilder(
      ...
      builder: (gallerySavePath) {
        ...
        final albumName = usingGallery
            ? Uri.decodeFull(
                gallerySavePath.substring(kGallerySavePathGalleryPrefix.length))
            : '';
        return SegmentedSettingWidget(
            ...
            children: {
              GallerySavePathOrganizing.noSubfolders: (
                null,
                usingGallery
                    ? (albumName.isEmpty ? 'No album' : '"$albumName" album')
                    : 'No subfolders'
              ),
              GallerySavePathOrganizing.siteSubfolders: (
                null,
                usingGallery ? 'Per-site albums' : 'Per-site subfolders'
              ),
              GallerySavePathOrganizing.boardSubfolders: (
                null,
                usingGallery ? 'Per-board albums' : 'Per-board subfolders'
              ),
              if (!usingGallery) ...{
                GallerySavePathOrganizing.boardAndThreadSubfolders: (
                  null,
                  'Per-board and per-thread subfolders'
                ),
                GallerySavePathOrganizing.boardAndThreadNameSubfolders: (
                  null,
                  'Per-board and per-thread (with name) subfolders'
                ),
                GallerySavePathOrganizing.threadNameSubfolders: (
                  null,
                  'Per-thread (with name) subfolders'
                )
              },
              GallerySavePathOrganizing.siteAndBoardSubfolders: (
                null,
                usingGallery
                    ? 'Per-site+board albums'
                    : 'Per-site and per-board subfolders'
              ),
              if (!usingGallery) ...{
                GallerySavePathOrganizing.siteBoardAndThreadSubfolders: (
                  null,
                  'Per-site, per-board, and per-thread subfolders'
                ),
                GallerySavePathOrganizing.siteBoardAndThreadNameSubfolders: (
                  null,
                  'Per-site, per-board, and per-thread (with name) subfolders'
                ),
                GallerySavePathOrganizing.siteAndThreadNameSubfolders: (
                  null,
                  'Per-site and per-thread (with name) subfolders'
                )
              }
            },
  ...
  const SwitchSettingWidget(
      ...
      helpText:
          'Use a machine-learning captcha solving model which is hosted on a web server to provide better captcha solver guesses. This means the captchas you open will be sent to a first-party web service for predictions. No information will be retained.',
      setting:
  ...
  const SwitchSettingWidget(
      ...
      helpText:
          'Cloud captcha solutions will be submitted directly without showing a popup and asking for confirmation.',
      setting: SettingWithFallback(
          Settings.useHeadlessCloudCaptchaSolverSetting, false),
  ...
  const SwitchSettingWidget(
      ...
      icon: CupertinoIcons.textformat,
      helpText:
          'Send the captcha images you solve to a database to improve the automated solver. No other information about your posts will be collected.',
  ...
  SwitchSettingWidget(
      ...
      helpText:
          'Crash stack traces and uncaught exceptions will be used to help fix bugs. No personal information will be collected.',
      setting: CustomImmutableSetting(
        reader: (context) => Settings.instance.isCrashlyticsCollectionEnabled,
        watcher: (context) => context
            .select<Settings, bool>((s) => s.isCrashlyticsCollectionEnabled),
  ...
  const SwitchSettingWidget(
      ...
      helpText:
          'Links to sites such as YouTube will show the thumbnail and title of the page instead of the link URL.',
  ...
  const SwitchSettingWidget(
      ...
      helpText:
          'Uploaded files will be re-encoded to prevent matching against other files.',
  ...
  SliderSettingWidget(
    ...
    setting: const MappedSetting(Settings.downloadInterFileDelayMsSetting,
        FieldMappers.toDouble, FieldMappers.toInt),
    min: 0,
    max: 2000,
    step: 50,
    textFormatter: (s) =>
  ...
  PopupSubpageSettingWidget(
      ...
      settings: [
        ...
        const SwitchSettingWidget(
          description: 'Auto-upload new downloads to CopyParty',
          icon: CupertinoIcons.cloud_upload,
          setting: Settings.copypartyAutoUploadSetting,
        ),
        ImmutableButtonSettingWidget(
          description: 'Server URL',
          icon: CupertinoIcons.globe,
          setting: Settings.copypartyServerUrlSetting,
          builder: (url) => Text(url.isEmpty ? 'Not set' : url,
          ...
          onPressed: (context, url, setUrl) async {
            ...
            if (save == true) {
              ...
              // Warn if remote/mixed threads exist — their files will become
              // unreachable once the server URL changes (D2).
              final remoteCount = ThreadDownloadService.instance.allDownloads
                  .where((d) =>
                      d.effectiveStorageLocation ==
                          ThreadStorageLocation.remote ||
                      d.effectiveStorageLocation == ThreadStorageLocation.mixed)
                  .length;
              if (remoteCount > 0 && context.mounted) {
                final confirmed = await showAdaptiveDialog<bool>(
                  context: context,
                  builder: (ctx) => AdaptiveAlertDialog(
                    title: const Text('Server URL changed'),
                    content: Text(
                      '$remoteCount '
                      '${remoteCount == 1 ? 'thread has files' : 'threads have files'} '
                      'stored on the current CopyParty server. Changing the URL '
                      'will make those files inaccessible through the app.',
                    ),
                    actions: [
                      AdaptiveDialogAction(
                        child: const Text('Cancel'),
                ...
                if (confirmed == true) setUrl(newUrl);
              } else {
                setUrl(newUrl);
              }
            }
        ...
        ImmutableButtonSettingWidget(
          ...
          builder: (path) => Text(path.isEmpty ? 'Not set' : path,
              overflow: TextOverflow.ellipsis),
          onPressed: (context, path, setPath) async {
            ...
            final save = await showAdaptiveDialog<bool>(
                ...
                builder: (context) => AdaptiveAlertDialog(
                        ...
                        content:
                          ...
                          const SizedBox(height: 10),
                          const Text(
                              'Root path on the server where files will be uploaded'),
                          const SizedBox(height: 10),
                          AdaptiveTextField(
                            autofocus: true,
                            controller: controller,
            ...
            if (save == true) {
              ...
              // Warn if remote/mixed threads exist — their stored remote paths use
              // the old destRoot and will break after the change (D2).
              final remoteCount = ThreadDownloadService.instance.allDownloads
                  .where((d) =>
                      d.effectiveStorageLocation ==
                          ThreadStorageLocation.remote ||
              ...
              if (remoteCount > 0 && context.mounted) {
                final confirmed = await showAdaptiveDialog<bool>(
                  context: context,
                  builder: (ctx) => AdaptiveAlertDialog(
                    title: const Text('Destination folder changed'),
                    content: Text(
                      '$remoteCount '
                      '${remoteCount == 1 ? 'thread has files' : 'threads have files'} '
                      'stored under the current destination folder. Changing it '
                      'will make those files inaccessible through the app.',
                    ),
                    actions: [
                      AdaptiveDialogAction(
                ...
                if (confirmed == true) setPath(newPath);
              } else {
                setPath(newPath);
              }
            }
            ...
            if (serverUrl.isEmpty) {
              showAdaptiveDialog(
                ...
                builder: (context) => AdaptiveAlertDialog(
                  ...
                  content:
                      const Text('Configure the CopyParty server URL first.'),
                  actions: [
                    AdaptiveDialogAction(
                        isDefaultAction: true,
                        onPressed: () => Navigator.pop(context),
  ...
  SettingHiding(
      hidden: const MappedSetting(
          Settings.separateWiFiAndCellularCookiesSetting,
          FieldMappers.invert,
          FieldMappers.invert),
      setting: SimpleButtonSettingWidget(
  ...
  ImmutableButtonSettingWidget(
      ...
      builder: (userAgent) =>
          userAgent != null ? const Text('Edit*') : const Text('Set'),
            ...
            builder: (context) => StatefulBuilder(
                builder: (context, setDialogState) => AdaptiveAlertDialog(
                        ...
                        content:
                          ...
                          const SizedBox(height: 10),
                          const Text(
                              'Cloudflare clearance will probably only work on the default user-agent'),
                          const SizedBox(height: 10),
                          Visibility.maintain(
                              visible: (userAgent?.isEmpty ?? true),
                              child: const Text(
                                  'You are currently using your system\'s default User-Agent (recommended)')),
                          const SizedBox(height: 10),
                          AdaptiveTextField(
                              autofocus: true,
                              controller: controller,
                        ...
                        actions: [
                          ...
                          AdaptiveDialogAction(
                              ...
                              onPressed: () {
                                ...
                                final idx =
                                    userAgents.indexOf(controller.text) + 1;
                                userAgent = controller.text =
                                    userAgents[idx % userAgents.length];
                                setDialogState(() {});
                              }),
                          AdaptiveDialogAction(
                              isDefaultAction: true,
  ...
  SimpleButtonSettingWidget(
      ...
      onPressed: (context) async {
        final choice = await showAdaptiveDialog<
                Future<void> Function(BuildContext)>(
            context: context,
            barrierDismissible: true,
            builder: (context) => AdaptiveAlertDialog(actions: [
                  AdaptiveDialogAction(
                      child: const Text('Share...'),
                      onPressed: () =>
                          Navigator.pop<Future<void> Function(BuildContext)>(
                              context,
                              LoggingInterceptor.instance.reportViaShareSheet)),
                  AdaptiveDialogAction(
                      child: const Text('Save as...'),
                      onPressed: () =>
                          Navigator.pop<Future<void> Function(BuildContext)>(
                              context,
                              LoggingInterceptor.instance.reportViaSaveAs)),
                  AdaptiveDialogAction(
                      child: const Text('Email to developer'),
                      onPressed: () =>
                          Navigator.pop<Future<void> Function(BuildContext)>(
                              context,
                              (_) => LoggingInterceptor.instance
                                  .reportViaEmail())),
                  AdaptiveDialogAction(
                      child: const Text('Cancel'),
                      onPressed: () => Navigator.pop(context))
  ...
  SimpleButtonSettingWidget(
      ...
      onPressed: (context) async {
        ...
        try {
          final file =
              await modalLoad(context, 'Exporting...', (_) => exportJson());
          if (!context.mounted) {
            return;
          }
          await alert(context, 'Warning',
              'This is NOT a backup. You won\'t be able to restore it if you reinstall the app. It\'s for your personal curiosity only.',
              barrierDismissible: false);
          if (!context.mounted) {
            return;
          }
  ...
  SimpleButtonSettingWidget(
      ...
      onPressed: (context) async {
        final globalRect = context.globalPaintBounds;
        final attachmentsSizeInBytes =
            await modalLoad(context, 'Scanning...', (_) async {
          int total = 0;
          await for (final file
              in Persistence.savedAttachmentsDirectory.list(recursive: true)) {
            ...
            builder: (context) => AdaptiveAlertDialog(
                    ...
                    content: StatefulBuilder(
                        builder: (context, setDialogState) =>
                            Column(mainAxisSize: MainAxisSize.min, children: [
                              if (attachmentsSizeInBytes > 0)
                                Row(children: [
                                  Expanded(
                                      child: Text(
                                          'Include saved attachments (${formatFilesize(attachmentsSizeInBytes)})')),
                                  AdaptiveSwitch(
                    ...
                    actions: [
                      AdaptiveDialogAction(
                          onPressed: () =>
                              Navigator.pop<Future<void> Function(File)>(
                                  context,
                                  (File file) => shareOne(
                                      context: context,
                                      text: file.path,
        ...
        try {
          final file = await modalLoad(
              ...
              'Exporting...',
              (_) => export(
                  includeSavedAttachments: attachmentsSizeInBytes > 0 &&
                      _exportIncludeSavedAttachments,
  ...
  SimpleButtonSettingWidget(
      ...
      onPressed: (context) async {
        ...
        try {
          final log = await modalLoad(
              context, 'Importing...', (_) => import(File(picked)));
                      ...
                      message: Column(
                          ...
                          children: log
                              .map((e) => Padding(
                                  ...
                                  child: switch (e) {
                                    ImportLogSummary() =>
                                      ImportLogSummaryWidget(summary: e),
                                    ImportLogFailure() =>
                                      ImportLogFailureWidget(failure: e),
                                    ImportLogConflict() =>
                                      ImportLogConflictWidget(conflict: e)
                                  }))
                              .toList()),
                      actions: [
