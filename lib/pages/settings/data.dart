  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, child) {
        ...
        confirmDelete(
          List<PersistentThreadState> toDelete, {
          String itemType = 'thread',
        }) async {
        ...
        final savedThreads = threadStateBox.values
            .where((t) => t.savedTime != null)
            .toList();
            ...
            children: [
              ...
              TableRow(
                children: [
                  ...
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    child: Text(
                      watchedThreads.length.toString(),
                      textAlign: TextAlign.right,
                    ),
                  ),
                  AdaptiveIconButton(
              ...
              ...oldThreadRows.map((entry) {
                return TableRow(
                  children: [
                    Text(
                      'Over ${entry.$1} days old',
                      textAlign: TextAlign.left,
                    ),
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 16),
                      child: Text(
                        entry.$2.length.toString(),
                        textAlign: TextAlign.right,
                      ),
                    ),
                    AdaptiveIconButton(
    ...
    return Row(
      children: [
        ...
        Flexible(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [Text(summary.newCount.toString()), const Text('New')],
          ),
        ),
        Flexible(
          flex: 1,
                ...
                child: AdaptiveThinButton(
                  onPressed: accepted
                      ...
                      : () {
                          widget.conflict.conflict.set(
                            widget.conflict.yours,
                            widget.conflict.conflict.get(
                              widget.conflict.theirs,
                            ),
                          );
                          // Hack
                          if (widget.conflict.yours is SavedSettings) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text.rich(
                        _importFieldValue(
                          widget.conflict.conflict.get(widget.conflict.theirs),
                        ),
                      ),
                    ],
                  ),
...
class _CopyPartyImportDialog extends StatefulWidget {
  ...
  @override
  State<_CopyPartyImportDialog> createState() => _CopyPartyImportDialogState();
}
...
class _CopyPartyImportDialogState extends State<_CopyPartyImportDialog> {
  ...
  void initState() {
    super.initState();
    _sub = ThreadDownloadService.instance.importThreadsFromCopyparty().listen(
      (p) {
        if (mounted) setState(() => _progress = p);
      },
      onError: (e) {
        if (mounted) {
          setState(
            () => _progress = ImportFromCopypartyProgress(
        ...
        child: Column(
          ...
          children: [
            if (p == null)
              ...
              Text(
                'Scanning\u2026 found ${p.found}, skipped ${p.skipped}${p.errors > 0 ? ', ${p.errors} errors' : ''}',
              ),
              const SizedBox(height: 10),
...
final dataSettings = [
    ...
    onPressed: (context, currentPath, setPath) async {
      ...
      if (newPath != null) {
        if (newPath.startsWith(kGallerySavePathGalleryPrefix)) {
          final newSavePathOrganizing = switch (Settings
              .gallerySavePathOrganizingSetting
              .read(context)) {
          ...
          if (newSavePathOrganizing != null) {
            final reallyChange = await confirm(
              context,
              'Warning',
              content:
            ...
            Settings.gallerySavePathOrganizingSetting.write(
              context,
              newSavePathOrganizing.$1,
            );
          }
        }
        setPath(newPath);
  ...
  SettingBuilder(
    ...
    builder: (gallerySavePath) {
      ...
      final usingGallery = gallerySavePath.startsWith(
        kGallerySavePathGalleryPrefix,
      );
  ...
  const SwitchSettingWidget(
    ...
    setting: SettingWithFallback(
      Settings.useHeadlessCloudCaptchaSolverSetting,
      false,
    ),
  ...
  SwitchSettingWidget(
    ...
    setting: CustomImmutableSetting(
      ...
      watcher: (context) => context.select<Settings, bool>(
        (s) => s.isCrashlyticsCollectionEnabled,
      ),
  ...
  SettingHiding(
    ...
    setting: SimpleButtonSettingWidget(
      ...
      onPressed: (context) => Navigator.of(context).push(
        adaptivePageRoute(
          builder: (context) =>
              CookieSettingsPage(jar: Persistence.currentCookies),
        ),
      ),
    ),
  ),
  SettingHiding(
    ...
    setting: SimpleButtonSettingWidget(
      ...
      onPressed: (context) => Navigator.of(context).push(
        adaptivePageRoute(
          builder: (context) =>
              CookieSettingsPage(jar: Persistence.wifiCookies),
        ),
      ),
    ),
  ),
  SettingHiding(
    ...
    setting: SimpleButtonSettingWidget(
      ...
      onPressed: (context) => Navigator.of(context).push(
        adaptivePageRoute(
          builder: (context) =>
              CookieSettingsPage(jar: Persistence.cellularCookies),
        ),
      ),
    ),
  ),
  ...
  SimpleButtonSettingWidget(
    ...
    onPressed: (context) async {
      final choice =
          await showAdaptiveDialog<Future<void> Function(BuildContext)>(
            context: context,
            barrierDismissible: true,
            builder: (context) => AdaptiveAlertDialog(
              actions: [
                ...
                AdaptiveDialogAction(
                  ...
                  onPressed: () =>
                      Navigator.pop<Future<void> Function(BuildContext)>(
                        context,
                        (_) => LoggingInterceptor.instance.reportViaEmail(),
                      ),
                ),
                AdaptiveDialogAction(
                  child: const Text('Cancel'),
  ...
  SimpleButtonSettingWidget(
    ...
    onPressed: (context) async {
      final globalRect = context.globalPaintBounds;
      final attachmentsSizeInBytes = await modalLoad(context, 'Scanning...', (
        _,
      ) async {
        int total = 0;
        await for (final file in Persistence.savedAttachmentsDirectory.list(
          recursive: true,
        )) {
      ...
      final cb = await showAdaptiveDialog<Future<void> Function(File)>(
        ...
        builder: (context) => AdaptiveAlertDialog(
          ...
          actions: [
            AdaptiveDialogAction(
              onPressed: () => Navigator.pop<Future<void> Function(File)>(
                context,
                (File file) => shareOne(
                  context: context,
                  text: file.path,
      ...
      try {
        final file = await modalLoad(
          ...
          'Exporting...',
          (_) => export(
            includeSavedAttachments:
                attachmentsSizeInBytes > 0 && _exportIncludeSavedAttachments,
              ...
              children: log
                  .map(
                    (e) => Padding(
                      ...
                      padding: const EdgeInsets.all(8),
                      child: switch (e) {
                        ImportLogSummary() => ImportLogSummaryWidget(
                          summary: e,
                        ),
                        ImportLogFailure() => ImportLogFailureWidget(
                          failure: e,
                        ),
                        ImportLogConflict() => ImportLogConflictWidget(
                          conflict: e,
                        ),
                      },
                    ),
