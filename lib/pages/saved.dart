...
typedef SavedPageMasterDetailPanesState =
    MultiMasterDetailPage5State<
...
class _SavedPageState extends State<SavedPage> {
  ...
  final _watchedThreadsListKey = GlobalKey(
    debugLabel: '_SavedPageState._watchedThreadsListKey',
  );
  final _savedThreadsListKey = GlobalKey(
    debugLabel: '_SavedPageState._savedThreadsListKey',
  );
  final _savedPostsListKey = GlobalKey(
    debugLabel: '_SavedPageState._savedPostsListKey',
  );
  final _yourPostsListKey = GlobalKey(
    debugLabel: '_SavedPageState._yourPostsListKey',
  );
  ...
  void initState() {
    ...
    _downloadsBadgeUpdater = ThreadDownloadService.instance
        .watchAllChanges()
        .listen((_) {
  ...
  Future<PostThreadCombo?> _takeYourPost() async {
    ...
    for (final entry in _yourPostsLists.entries) {
      while (entry.value.isNotEmpty) {
        ...
        final state = entry.key.$1.persistence.getThreadStateIfExists(
          last.thread,
        );
        if (state == null) {
          // Something weird...
        ...
        heads.add(
          PostThreadCombo(imageboard: entry.key.$1, thread: thread, post: post),
        );
        break;
      }
    }
  ...
  static ContextMenuAction _makeFindSavedAttachmentInThreadAction({
    ...
    onPressed: () async {
      try {
        ...
        final (thread, postId) = await modalLoad(context, 'Finding...', (
          controller,
        ) async {
          final threadState = imageboard.persistence.getThreadStateIfExists(
            threadIdentifier,
          );
        ...
        Navigator.of(
          (poppedOut || innerContext == null) ? context : innerContext,
        ).push(
          adaptivePageRoute(
    ...
    return MultiMasterDetailPage5(
      ...
      paneCreator1: () => MultiMasterPane<ImageboardScoped<ThreadWatch>>(
          ...
          actions: [
            ...
            Builder(
              builder: (context) => CupertinoButton(
                padding: EdgeInsets.zero,
                child: Icon(
                  Settings.reverseWatchedThreadsSortingSetting.watch(context)
                      ? CupertinoIcons.sort_up
                      : CupertinoIcons.sort_down,
                ),
                onPressed: () => selectWatchedThreadsSortMethod(
                  context,
            ...
            Builder(
              builder: (context) {
                final onlyShowUnreadWatches = Settings
                    .onlyShowUnreadWatchesSetting
                    .watch(context);
        ...
        icon: AnimatedBuilder(
          ...
          builder: (context, _) => CachingBuilder(
            value: ImageboardRegistry.instance.imageboards.any(
              (i) => i.persistence.browserState.threadWatches.values.isNotEmpty,
            ),
        ...
        masterBuilder: (context, selected, setter) {
          ...
          return RefreshableList<ImageboardScoped<(ThreadWatch, Thread)>>(
            ...
            id: 'watched',
            minCacheExtent: settings.useCatalogGrid
            ...
            gridDelegate: settings.useCatalogGrid
                ? SliverGridDelegateWithMaxCrossAxisExtentWithCacheTrickery(
                    ...
                    childAspectRatio:
                        settings.catalogGridWidth / settings.catalogGridHeight,
                  )
                : null,
            staggeredGridDelegate:
                (settings.useCatalogGrid && settings.useStaggeredCatalogGrid)
                ...
                builder: (context) => AnimatedBuilder(
                  ...
                  builder: (context, _) => Wrap(
                    ...
                    children: thread_actions
                        .getWatchedThreadsActions(
                          context,
                          onMutate: _removeArchivedHack.didUpdate,
                        )
                        .map(
                          (a) => CupertinoButton(
                ...
                child: ContextMenu(
                  ...
                  actions: [
                    ...
                    ContextMenuAction(
                      ...
                      onPressed: () async {
                        await watch.imageboard.notifications.removeWatch(
                          watch.item.$1,
                        );
                        _watchedListController.update();
                        if (context.mounted) {
                          showUndoToast(
                            ...
                            message: 'Unwatched',
                            onUndo: () async {
                              await watch.imageboard.notifications.insertWatch(
                                watch.item.$1,
                              );
                              _watchedListController.update();
                            },
                    ...
                    ContextMenuAction(
                      ...
                      onPressed: () async {
                        final threadState = watch.imageboard.persistence
                            .getThreadState(watch.item.$1.threadIdentifier);
                        final unseenPostIds = threadState.unseenPostIds.data
                            .toSet();
                        final lastSeenPostId = threadState.lastSeenPostId;
                        threadState.unseenPostIds.data.clear();
                        threadState.lastSeenPostId = threadState.thread?.posts_
                            .fold<int>(0, (m, p) => math.max(m, p.id));
                        ...
                        if (context.mounted) {
                          showUndoToast(
                            ...
                            onUndo: () async {
                              threadState.unseenPostIds.data.addAll(
                                unseenPostIds,
                              );
                  ...
                  child: GestureDetector(
                    ...
                    child: ValueListenableBuilder(
                      valueListenable: watch.imageboard.persistence
                      ...
                      builder: (context, threadState, child) {
                        return NullableColorFiltered(
                          colorFilter:
                              ((threadState?.unseenReplyIdsToYouCount() ?? 0) >
                                  0)
                              ? ColorFilter.mode(
                          ...
                          child: ThreadRow(
                            ...
                            onThumbnailLoadError: (error, stackTrace) {
                              watch.imageboard.threadWatcher.fixBrokenThread(
                                watch.item.$1.threadIdentifier,
                              );
                            },
                              ...
                              final attachments = {
                                for (final w in _watchedListController.items)
                                  for (final attachment
                                      in w.item.imageboard.persistence
                                              .getThreadStateIfExists(
                                                w.item.item.$1.threadIdentifier,
                                              )
                                              ?.thread
                                              ?.attachments ??
                                          <Attachment>[])
                              ...
                              showGalleryPretagged(
                                ...
                                threads: {
                                  for (final item in attachments.entries)
                                    item.key.attachment: item.value.imageboard!
                                        .scope(item.value.thread!),
                                },
                                onThreadSelected: (t) {
                                  final x = _watchedListController.items
                                      .firstWhere(
                                        (w) =>
                                            w.item.imageboard == t.imageboard &&
                                            w.item.item.$1.threadIdentifier ==
                                                t.item.identifier,
                                      )
                                      .item;
                                ...
                                onChange: (attachment) {
                                  _watchedListController.animateTo(
                                    (p) =>
                                        p.imageboard == attachment.imageboard &&
                                        p.item.$1.board.toLowerCase() ==
                                            attachment.attachment.board
                                                .toLowerCase() &&
                                        p.item.$1.threadId == attachment.postId,
                                  );
                                },
      ...
      paneCreator2: () =>
          MultiMasterPane<ImageboardScoped<ThreadOrPostIdentifier>>(
                ...
                AnimatedBuilder(
                  ...
                  builder: (context, _) => CupertinoButton(
                    ...
                    onPressed: _threadListController.items.isEmpty
                        ? null
                        : () => thread_actions.unsaveAllSavedThreads(
                            context,
            ...
            icon: AnimatedBuilder(
              ...
              builder: (context, _) => CachingBuilder(
                value: Persistence.sharedThreadStateBox.values.any(
                  (s) => s.savedTime != null,
                ),
                builder: (value) => Builder(
                  builder: (context) => Icon(
              ...
              return RefreshableList<(PersistentThreadState, Thread)>(
                ...
                listUpdater: (options) async {
                  ...
                  final missing = await loader.initialize(
                    settings.reverseSavedThreadsSorting,
                  );
                      ...
                      child: GestureDetector(
                        ...
                        child: Builder(
                          builder: (context) => ThreadRow(
                            ...
                            onThumbnailLoadError: (error, stackTrace) {
                              state.imageboard?.threadWatcher.fixBrokenThread(
                                state.identifier,
                              );
                            },
                            semanticParentIds: const [-12],
                            onThumbnailTap: (initialAttachment) {
                              final attachments = _threadListController.items
                                  .expand(
                                    (item) => item.item.$2.attachments.map(
                              ...
                              showGalleryPretagged(
                                ...
                                threads: {
                                  for (final state
                                      in _threadListController.items)
                                    for (final attachment
                                        in state.item.$2.attachments)
                                      attachment: state.item.$1.imageboard!
                                          .scope(state.item.$2),
                                },
                                onThreadSelected: (t) => threadSetter(
                                  t.imageboard.scope(
                                    t.item.identifier.threadOrPostIdentifier,
                                  ),
                                ),
                                initialAttachment: initialAttachment,
                                onChange: (attachment) {
                                  _threadListController.animateTo(
                                    (p) =>
                                        ...
                                        p.$1.board.toLowerCase() ==
                                            attachment.attachment.board
                                                .toLowerCase() &&
                                        p.$1.id == attachment.postId,
                                  );
                                },
                                heroOtherEndIsBoxFitCover:
                                    settings.useCatalogGrid ||
          ...
          return RefreshableList<PostThreadCombo>(
            ...
            listUpdater: (options) async {
              ...
              for (final state in Persistence.sharedThreadStateBox.values) {
                ...
                final l = _yourPostsLists.putIfAbsent((
                  imageboard,
                  state.board,
                ), () => []);
            ...
            minUpdateDuration: Duration.zero,
            sortMethods: [
              (a, b) => b.post.time.toLocal().compareTo(a.post.time.toLocal()),
            ],
                ...
                child: Builder(
                  builder: (context) => PostRow(
                    ...
                    onThumbnailLoadError: (e, st) async {
                      await item.imageboard.threadWatcher.fixBrokenThread(
                        item.thread.identifier,
                      );
                    },
                    onThumbnailTap: (initialAttachment) {
                      final attachments = _yourPostsListController.items
                          .expand(
                            (item) => item.item.post.attachments.map(
                              (attachment) => TaggedAttachment(
                        ...
                        posts: {
                          for (final state in _yourPostsListController.items)
                            for (final attachment
                              ...
                              attachment: state.item.imageboard.scope(
                                state.item.post,
                              ),
                        },
                        initialAttachment: initialAttachment,
                          ...
                          _yourPostsListController.animateTo(
                            (p) =>
                                ...
                                p.post.board.toLowerCase() ==
                                    attachment.attachment.board.toLowerCase() &&
                                p.post.id == attachment.postId,
                          );
                        },
                        heroOtherEndIsBoxFitCover:
      ...
      paneCreator4: () => MultiMasterPane<ImageboardScoped<SavedPost>>(
            ...
            AnimatedBuilder(
              ...
              builder: (context, _) => CupertinoButton(
                ...
                onPressed: _postListController.items.isEmpty
                    ? null
                    : () => thread_actions.unsaveAllSavedPosts(
                        context,
        ...
        icon: AnimatedBuilder(
          ...
          builder: (context, _) => CachingBuilder(
            value: ImageboardRegistry.instance.imageboardsIncludingDev.any(
              (i) => i.persistence.savedPosts.isNotEmpty,
            ),
            builder: (value) => Builder(
              builder: (context) => Icon(
                value
          ...
          return RefreshableList<ImageboardScoped<(SavedPost, Thread)>>(
                ...
                onFixAbandonedForThreads: (threadsToDelete) async {
                  for (final thread in threadsToDelete) {
                    final postToDelete = thread
                        ...
                        .values
                        .tryFirstWhere(
                          (p) => p.post.threadIdentifier == thread.item,
                        );
                    if (postToDelete != null) {
                      thread.imageboard.persistence.unsavePost(
                        postToDelete.post,
                      );
                    }
                  }
            ...
            listUpdater: (options) async {
              final savedPosts = ImageboardRegistry
                  .instance
                  .imageboardsIncludingDev
                  .expand((i) => i.persistence.savedPosts.values.map(i.scope))
                  .toList();
              final pairs = await Future.wait(
                savedPosts.map((s) async {
                  return (
                    s,
                    await s.imageboard.persistence
                        .getThreadStateIfExists(s.item.post.threadIdentifier)
                        ?.getThread(),
                  );
                }),
              );
                ...
                child: ChangeNotifierProvider<PostSpanZoneData>(
                  ...
                  child: Builder(
                    builder: (context) => PostRow(
                      ...
                      isSelected: selected(
                        context,
                        savedPost.imageboard.scope(savedPost.item.$1),
                      ),
                      onTap: () =>
                          setter(savedPost.imageboard.scope(savedPost.item.$1)),
                      showBoardName: true,
                      showSiteIcon: true,
                      hideThumbnails: options.hideThumbnails,
                      baseOptions: PostSpanRenderOptions(
                      ...
                      onThumbnailLoadError: (e, st) async {
                        ...
                        if (firstThread != threadState!.thread ||
                            threadState.thread?.archiveName != null) {
                          savedPost.item.$1.post = threadState.thread!.posts
                              .firstWhere(
                                (p) => p.id == savedPost.item.$1.post.id,
                              );
                          savedPost.imageboard.persistence.didUpdateSavedPost();
                        }
                      },
                      onThumbnailTap: (initialAttachment) {
                        final attachments = _postListController.items
                            .expand(
                              (item) => item.item.item.$1.post.attachments.map(
                                (attachment) => TaggedAttachment(
                                  imageboard: item.item.imageboard,
                                  attachment: attachment,
                                  semanticParentIds: [-2],
                        ...
                        showGalleryPretagged(
                          ...
                          posts: {
                            for (final state in _postListController.items)
                              for (final attachment
                                  in state.item.item.$1.post.attachments)
                                attachment: state.item.imageboard.scope(
                          ...
                          initialAttachment: initialAttachment,
                          onChange: (attachment) {
                            _postListController.animateTo(
                              (p) =>
                                  p.imageboard == attachment.imageboard &&
                                  p.item.$1.post.board.toLowerCase() ==
                                      attachment.attachment.board
                                          .toLowerCase() &&
                                  p.item.$1.post.id == attachment.postId,
                            );
                          },
                          heroOtherEndIsBoxFitCover:
                              Settings.instance.squareThumbnails,
      ...
      paneCreator5: () => MultiMasterPane<ImageboardScoped<SavedAttachment>>(
            ...
            Builder(
              builder: (context) => AnimatedBuilder(
                ...
                builder: (context, _) => CupertinoButton(
                  padding: EdgeInsets.zero,
                  onPressed:
                      ImageboardRegistry.instance.imageboards.any(
                        (i) => i.persistence.savedAttachments.isNotEmpty,
                      )
                      ? () async {
                          final toDelete = ImageboardRegistry
                              ...
                              .imageboards
                              .expand(
                                (i) => i.persistence.savedAttachments.values
                                    .map(i.scope),
                              )
                              .toList();
                          ...
                          final imageboards = toDelete
                              .map((i) => i.imageboard)
                              .toSet();
                          for (final item in toDelete) {
                            item.imageboard.persistence.savedAttachments.remove(
                              item.item.attachment.globalId,
                            );
                          }
                          for (final imageboard in imageboards) {
                            imageboard.persistence.savedAttachmentsListenable
                                .didUpdate();
        ...
        masterBuilder: (context, selected, setter) =>
            RefreshableList<ImageboardScoped<SavedAttachment>>(
              id: 'savedAttachments',
              ...
              listUpdater: (options) async {
                ...
                for (final imageboard
                  ...
                  for (final entry
                      in imageboard.persistence.savedAttachments.entries
                          .toList()) {
                    if (!imageboard.persistence.savedAttachments.containsKey(
                      entry.key,
                    )) {
                ...
                builder: (context, missing, _) => MissingAttachmentsControls(
                  ...
                  onFixAbandonedForAttachments: (attachments) async {
                    for (final a in attachments) {
                      a.imageboard.persistence.deleteSavedAttachment(
                        a.item.attachment,
                      );
                    }
                  },
              ...
              itemBuilder: (context, item, options) => Builder(
                builder: (context) {
                  makeController() => AttachmentViewerController(
                    ...
                    context: context,
                    imageboard: item.imageboard,
                    isDownloaded: _downloadedAttachments.contains(
                  ...
                  return ImageboardScope(
                    ...
                    child: CupertinoInkwell(
                      ...
                      child: ContextMenu(
                        actions: [
                          ...
                          ContextMenuAction(
                            ...
                            onPressed: () async {
                              final controller = makeController();
                              final text = controller.goodImagePublicSource
                                  .toString();
                              shareOne(
                                context: context,
                                text: text,
                          ...
                          ...buildImageSearchActions(context, item.imageboard, [
                            item.item.attachment,
                          ]),
                      ...
                      onPressed: () async {
                        if (context.read<MasterDetailHint?>()?.currentValue ==
                            null) {
            ...
            child = ImageboardScope(
              ...
              child: Builder(
                builder: (innerContext) => GalleryPage(
                  ...
                  onChange: (a) {
                    final originalL = _savedAttachmentsController.items
                        .tryFirstWhere(
                          (l) => l.item.item.attachment == a.attachment,
                        )
                        ?.item;
                    _savedAttachmentsController.animateToIfOffscreen(
                      (l) => l.item.attachment == a.attachment,
                      alignment: 0.5,
                    );
                  ...
                  additionalContextMenuActionsBuilder: (attachment) => [
                    _makeFindSavedAttachmentInThreadAction(
                      ...
                      imageboard: ImageboardRegistry.instance.getImageboard(
                        imageboardIds.entries
                            .tryFirstWhere(
                              (e) =>
                                  e.value == attachment.semanticParentIds.last,
                            )
                            ?.key,
                      ),
                    ),
                      ...
                      child: TimedRebuilder(
                        ...
                        function: () {
                          ...
                          return w.updatingNow != null
                              ? null
                              : now.difference(w.lastUpdate!).inSeconds /
                                    w.nextUpdate!
                        ...
                        builder: (context, value) {
                          return LinearProgressIndicator(
                            ...
                            backgroundColor:
                                ChanceTheme.primaryColorWithBrightness20Of(
                                  context,
                                ),
                            minHeight: 8,
                          );
  ...
  Widget build(BuildContext context) => _MissingControls(
    ...
    fixer: (thread) async {
      try {
        ...
        if (newThread == null) {
          try {
            newThread = await thread.imageboard.site.getThread(
              thread.item,
              priority: RequestPriority.interactive,
            );
          } on ThreadNotFoundException {
        ...
        if (newThread != null) {
          final state = thread.imageboard.persistence.getThreadState(
            thread.item,
          );
      ...
      try {
        try {
          ...
          await attachment.imageboard.site.client.download(
            ...
            options: Options(
              headers: {
                ...attachment.imageboard.site.getHeaders(
        ...
        if (threadId != null) {
          final threadIdentifier = ThreadIdentifier(
            attachment.item.attachment.board,
            threadId,
          );
          try {
            ...
            final archivedThread = await attachment.imageboard.site
                .getThreadFromArchive(
                  ...
                  customValidator: (thread) async {
                    ...
                    final response = await attachment.imageboard.site.client
                        .head(
                          found.url,
                          options: Options(
                            headers: {
                    ...
                    if (response.redirects.isNotEmpty) {
                      redirectUrls[found.url] = response.redirects.last.location
                          .toString();
                    } else if (response.headers.value(
