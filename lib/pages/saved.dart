...
class PostThreadCombo {
  ...
  final Thread thread;
  PostThreadCombo(
      {required this.imageboard, required this.post, required this.thread});
...
typedef SavedPageMasterDetailPanesState = MultiMasterDetailPage5State<
    ImageboardScoped<ThreadWatch>,
    ImageboardScoped<ThreadOrPostIdentifier>,
    PostThreadCombo,
    ImageboardScoped<SavedPost>,
    ImageboardScoped<SavedAttachment>>;

/// As different archives may have slight mismatches
({Post post, Attachment attachment})? _findMatchingAttachment(
...
bool _isLiveDownload(DownloadedThread t) {
  ...
  if (t.rawIsLockedOnServer == null) {
    final tsKey = '${t.imageboardKey}/${t.board.toLowerCase()}/${t.threadId}';
    if (Persistence.sharedThreadStateBox.get(tsKey)?.thread?.isLocked == true) {
      return false;
    }
  }
  return true;
...
class _SavedThreadsByTitleLoader
    extends _SavedThreadsLoaderImpl<(PersistentThreadState, String)> {
  static final _cache = <PersistentThreadState, String>{};
  @override
  Future<(PersistentThreadState, String)?> _initializeImpl(
      PersistentThreadState state) async {
    ...
    final title = _cache[state] =
        thread.title ?? thread.posts_.tryFirst?.buildText() ?? '';
    return (state, title);
  }
  ...
  @override
  PersistentThreadState _takeSavedThreadImpl(
          (PersistentThreadState, String) entry) =>
      entry.$1;
}

class _SavedThreadsByLastPostTimeLoader
    extends _SavedThreadsLoaderImpl<(PersistentThreadState, DateTime)> {
  static final _cache = <PersistentThreadState, DateTime>{};
  @override
  Future<(PersistentThreadState, DateTime)?> _initializeImpl(
      PersistentThreadState state) async {
    final cached = _cache[state];
    if (cached != null) {
      return (state, cached);
  ...
  @override
  PersistentThreadState _takeSavedThreadImpl(
          (PersistentThreadState, DateTime) entry) =>
      entry.$1;
}

class _SavedThreadsBySavedTimeLoader
    extends _SavedThreadsLoaderImpl<PersistentThreadState> {
  static final noDate = DateTime.fromMillisecondsSinceEpoch(0);
  @override
  Future<PersistentThreadState?> _initializeImpl(
          PersistentThreadState state) async =>
      state;
  @override
  void _sortImpl(bool reverse) {
    if (reverse) {
      // Latest saved first
      _list.sort(
          (a, b) => (b.savedTime ?? noDate).compareTo(a.savedTime ?? noDate));
    } else {
      // Earliest saved first
      _list.sort(
          (a, b) => (a.savedTime ?? noDate).compareTo(b.savedTime ?? noDate));
    }
  }

  @override
  PersistentThreadState _takeSavedThreadImpl(PersistentThreadState entry) =>
      entry;
}
...
class _SavedThreadsByThreadPostTimeLoader implements _SavedThreadsLoader {
  ...
  Future<(PersistentThreadState, Thread)?> takeSavedThread() async {
    ...
    for (final head in heads) {
      if (latestHead == null ||
          (_reverse
              ? head.$2.time.isBefore(latestHead.$2.time)
              : head.$2.time.isAfter(latestHead.$2.time))) {
        latestHead = head;
      }
    }
...
class SavedPage extends StatefulWidget {
  ...
  const SavedPage(
      {required this.masterDetailKey, this.initialThreadWatch, Key? key})
      : super(key: key);
...
class _SavedPageState extends State<SavedPage> {
  late final RefreshableListController<ImageboardScoped<(ThreadWatch, Thread)>>
      _watchedListController;
  late final RefreshableListController<(PersistentThreadState, Thread)>
      _threadListController;
  late final RefreshableListController<ImageboardScoped<(SavedPost, Thread)>>
      _postListController;
  late final RefreshableListController<PostThreadCombo>
      _yourPostsListController;
  final _watchedThreadsListKey =
      GlobalKey(debugLabel: '_SavedPageState._watchedThreadsListKey');
  final _savedThreadsListKey =
      GlobalKey(debugLabel: '_SavedPageState._savedThreadsListKey');
  final _savedPostsListKey =
      GlobalKey(debugLabel: '_SavedPageState._savedPostsListKey');
  final _yourPostsListKey =
      GlobalKey(debugLabel: '_SavedPageState._yourPostsListKey');
  final _savedAttachmentsAnimatedBuilderKey = GlobalKey(
      debugLabel: '_SavedPageState._savedAttachmentsAnimatedBuilderKey');
  late final RefreshableListController<ImageboardScoped<SavedAttachment>>
      _savedAttachmentsController;
  late final EasyListenable _removeArchivedHack;
  late final ValueNotifier<List<ImageboardScoped<ThreadIdentifier>>>
      _missingWatchedThreads;
  late final ValueNotifier<List<ImageboardScoped<ThreadIdentifier>>>
      _missingSavedThreads;
  late final ValueNotifier<List<ImageboardScoped<ThreadIdentifier>>>
      _missingSavedPostsThreads;
  late final ValueNotifier<List<ImageboardScoped<ThreadIdentifier>>>
      _missingYourPostsThreads;
  late final ValueNotifier<List<ImageboardScoped<SavedAttachment>>>
      _missingSavedAttachments;
  ...
  late final ValueNotifier<ImageboardScoped<ThreadOrPostIdentifier>?>
      _savedThreadsValueInjector;
  ...
  void initState() {
    ...
    _downloadsBadgeUpdater =
        ThreadDownloadService.instance.watchAllChanges().listen((_) {
      _downloadsBadgeNotifier.value = ThreadDownloadService
          .instance.downloadsBox.values
  ...
  Future<PostThreadCombo?> _takeYourPost() async {
    ...
    for (final entry in _yourPostsLists.entries) {
      while (entry.value.isNotEmpty) {
        ...
        final state =
            entry.key.$1.persistence.getThreadStateIfExists(last.thread);
        if (state == null) {
          // Something weird...
          entry.value.removeLast();
          continue;
        ...
        heads.add(PostThreadCombo(
            imageboard: entry.key.$1, thread: thread, post: post));
        break;
      }
    }
  ...
  Widget _placeholder(String message) {
    return AdaptiveScaffold(body: Center(child: Text(message)));
  }

  void _onSavedThreadsHistorySearch(String query) {
    widget.masterDetailKey.currentState!.masterKey.currentState!.push(
        adaptivePageRoute(
            builder: (context) => ValueListenableBuilder(
                valueListenable: _savedThreadsValueInjector,
                builder: (context,
                    ImageboardScoped<ThreadOrPostIdentifier>? selectedResult,
                    child) {
                  return HistorySearchPage(
                      ...
                      onResultSelected:
                          widget.masterDetailKey.currentState!.setValue2);
                }),
            settings: dontAutoPopSettings));
  }

  void _onYourPostsHistorySearch(String query) {
    widget.masterDetailKey.currentState!.masterKey.currentState!
            ...
            builder: (context) => ValueListenableBuilder(
                ...
                builder: (context, PostThreadCombo? selectedResult, child) {
                  ...
                  return HistorySearchPage(
                      ...
                      selectedResult: post == null
                          ? null
                          : selectedResult?.imageboard.scope(post),
                      onResultSelected: (result) async {
                        if (result == null) {
                          widget.masterDetailKey.currentState!.setValue3(null);
                          return;
                        }
                        final thread = await result.imageboard.persistence
                            .getThreadStateIfExists(result.item.thread)
                            ?.getThread();
                        if (thread == null) {
                          return;
                        }
                        final postId =
                            result.item.postId ?? result.item.threadId;
                        final post =
                            thread.posts.tryFirstWhere((p) => p.id == postId);
                        if (post == null) {
                          return;
                        }
                        widget.masterDetailKey.currentState!.setValue3(
                            PostThreadCombo(
      ...
      ContextMenuAction(
          ...
          onPressed: () async {
            try {
              ...
              final (thread, postId) =
                  await modalLoad(context, 'Finding...', (controller) async {
                final threadState = imageboard.persistence
                    .getThreadStateIfExists(threadIdentifier);
                Thread? thread = await threadState?.getThread();
                if (thread == null) {
                  try {
                    thread = await imageboard.site.getThread(threadIdentifier,
                        priority: RequestPriority.interactive);
                  } on ThreadNotFoundException {
                    thread = await imageboard.site.getThreadFromArchive(
                        threadIdentifier,
                        priority: RequestPriority.interactive,
                        customValidator: (t) async {
                ...
                final postId =
                    _findMatchingAttachment(thread, attachment)?.post.id;
              ...
              Navigator.of((poppedOut || innerContext == null)
                      ? context
                      : innerContext)
                  .push(adaptivePageRoute(
  ...
  Widget build(BuildContext context) {
    ...
    final persistencesAnimation = Listenable.merge(ImageboardRegistry
        .instance.imageboards
        .map((x) => x.persistence)
        .toList());
    final threadStateBoxesAnimation = Persistence.sharedThreadStateListenable;
    final savedPostsNotifiersAnimation = Listenable.merge(ImageboardRegistry
        .instance.imageboardsIncludingDev
        .map((i) => i.persistence.savedPostsListenable)
        .toList());
    final savedAttachmentsNotifiersAnimation = Listenable.merge(
        ImageboardRegistry.instance.imageboardsIncludingDev
            .map((i) => i.persistence.savedAttachmentsListenable)
            .toList());
    final imageboardIds = <String, int>{};
    return MultiMasterDetailPage5(
        id: 'saved',
        key: widget.masterDetailKey,
        paneCreator1: () => MultiMasterPane<ImageboardScoped<ThreadWatch>>(
            initialValue: widget.initialThreadWatch,
            navigationBar:
                AdaptiveBar(title: const Text('Watched Threads'), actions: [
              Builder(
                  builder: (context) => ValueListenableBuilder(
                  ...
                  builder: (context) => CupertinoButton(
                      ...
                      onPressed: () => showAdaptiveModalPopup(
                          ...
                          builder: (popupContext) => AdaptiveActionSheet(
                                  actions: [
                                    ...thread_actions
                                        .getWatchedThreadsActions(context)
              ...
              Builder(
                  builder: (context) => CupertinoButton(
                      padding: EdgeInsets.zero,
                      child: Icon(Settings.reverseWatchedThreadsSortingSetting
                              .watch(context)
                          ? CupertinoIcons.sort_up
                          : CupertinoIcons.sort_down),
                      onPressed: () => selectWatchedThreadsSortMethod(context,
                          onMutate: _watchedListController.update))),
              Builder(builder: (context) {
                final onlyShowUnreadWatches =
                    Settings.onlyShowUnreadWatchesSetting.watch(context);
                return CupertinoButton(
                    padding: EdgeInsets.zero,
                    child: Icon(onlyShowUnreadWatches
                        ? CupertinoIcons.asterisk_circle_fill
                        : CupertinoIcons.asterisk_circle),
                    onPressed: () {
                      final v = Settings.instance.onlyShowUnreadWatches =
                          !onlyShowUnreadWatches;
                      showToast(
                          context: context,
                          icon: v
                              ? CupertinoIcons.asterisk_circle_fill
            ...
            icon: AnimatedBuilder(
                ...
                builder: (context, _) => CachingBuilder(
                    value: ImageboardRegistry.instance.imageboards.any((i) => i
                        .persistence
                        .browserState
                        .threadWatches
                        .values
              ...
              return RefreshableList<ImageboardScoped<(ThreadWatch, Thread)>>(
                    ...
                    children: [
                      ...
                      ValueListenableBuilder(
                          ...
                          builder: (context, list, _) => MissingThreadsControls(
                              ...
                              onFixAbandonedForThreads:
                                  (threadsToDelete) async {
                                for (final thread in threadsToDelete) {
                                  await thread.imageboard.notifications
                                      .unsubscribeFromThread(thread.item);
                                }
                              }))
                    ],
                  ),
                  ...
                  listUpdater: (options) async {
                    ...
                    for (final item in list) {
                      final thread = item.imageboard.persistence
                          .getThreadStateIfExists(item.item.threadIdentifier)
                          ?.thread;
                      if (thread == null) {
                        missing.add(
                            item.imageboard.scope(item.item.threadIdentifier));
                      } else if (!settings.onlyShowUnreadWatches ||
                          item.shouldCountAsUnread) {
                    ...
                    _watchedListController
                        .waitForItemBuild(0)
                        .then((_) => _removeArchivedHack.didUpdate());
                    return out;
                  },
                  ...
                  id: 'watched',
                  minCacheExtent:
                      settings.useCatalogGrid ? settings.catalogGridHeight : 0,
                  gridDelegate: settings.useCatalogGrid
                      ? SliverGridDelegateWithMaxCrossAxisExtentWithCacheTrickery(
                          maxCrossAxisExtent: settings.catalogGridWidth,
                          childAspectRatio: settings.catalogGridWidth /
                              settings.catalogGridHeight)
                      : null,
                  staggeredGridDelegate: (settings.useCatalogGrid &&
                          settings.useStaggeredCatalogGrid)
                  ...
                  footer: Padding(
                      ...
                      child: Builder(
                          builder: (context) => AnimatedBuilder(
                              animation: TickerMode.of(context)
                                  ? Listenable.merge([
                                      _removeArchivedHack,
                  ...
                  itemBuilder: (itemContext, watch, options) {
                    ...
                    return ImageboardScope(
                        ...
                        child: ContextMenu(
                            ...
                            actions: [
                              if (openInNewTabZone != null)
                                ContextMenuAction(
                                    ...
                                    onPressed: () {
                                      openInNewTabZone.onWantOpenThreadInNewTab(
                                          watch.imageboard.key,
                                          watch.item.$1.threadIdentifier);
                                    }),
                              ContextMenuAction(
                                  child: const Text('Unwatch'),
                                  onPressed: () async {
                                    await watch.imageboard.notifications
                                        .removeWatch(watch.item.$1);
                                    _watchedListController.update();
                                    if (context.mounted) {
                                      showUndoToast(
                                          context: context,
                              ...
                              ContextMenuAction(
                                ...
                                onPressed: () async {
                                  final threadState = watch
                                      .imageboard.persistence
                                      .getThreadState(
                                          watch.item.$1.threadIdentifier);
                                  final unseenPostIds =
                                      threadState.unseenPostIds.data.toSet();
                                  final lastSeenPostId =
                                      threadState.lastSeenPostId;
                                  threadState.unseenPostIds.data.clear();
                                  threadState.lastSeenPostId =
                                      threadState.thread?.posts_.fold<int>(
                                          0, (m, p) => math.max(m, p.id));
                              ...
                              if (watch.imageboard.persistence
                            ...
                            contextMenuBuilderBuilder:
                                makeGeneralContextMenuBuilder,
                            child: GestureDetector(
                                behavior: HitTestBehavior.opaque,
                                child: ValueListenableBuilder(
                                    valueListenable: watch
                                        .imageboard.persistence
                                        .listenForPersistentThreadStateChanges(
                                            watch.item.$1.threadIdentifier),
                                    builder: (context, threadState, child) {
                                      return NullableColorFiltered(
                                          colorFilter: ((threadState
                                                          ?.unseenReplyIdsToYouCount() ??
                                                      0) >
                                                  0)
                                              ? ColorFilter.mode(
                                          ...
                                          child: ThreadRow(
                                              ...
                                              onThumbnailLoadError:
                                                  (error, stackTrace) {
                                                watch.imageboard.threadWatcher
                                                    .fixBrokenThread(watch.item
                                                        .$1.threadIdentifier);
                                              },
                                              ...
                                              onThumbnailTap:
                                                  (initialAttachment) {
                                                final attachments = {
                                                  for (final w in _watchedListController
                                                      .items)
                                                    for (final attachment in w
                                                            .item
                                                            .imageboard
                                                            .persistence
                                                            .getThreadStateIfExists(w
                                                                .item
                                                                .item
                                                                .$1
                                                                .threadIdentifier)
                                                      ...
                                                      TaggedAttachment(
                                                          ...
                                                          .imageboard
                                                          .persistence
                                                          .getThreadStateIfExists(w.item.item.$1.threadIdentifier)!
                                                };
                                                showGalleryPretagged(
                                                    ...
                                                    onThreadSelected: (t) {
                                                      final x = _watchedListController
                                                          .items
                                                          .firstWhere((w) =>
                                                              w.item.imageboard ==
                                                                  t
                                                    ...
                                                    initialAttachment:
                                                        initialAttachment,
                                                    onChange: (attachment) {
                                                      _watchedListController.animateTo((p) =>
                                                          p.imageboard ==
                                                              attachment
                                                                  .imageboard &&
                                                          p.item.$1.board
        ...
        paneCreator2: () => MultiMasterPane<ImageboardScoped<ThreadOrPostIdentifier>>(
            navigationBar: AdaptiveBar(title: const Text('Saved Threads'), actions: [
              AnimatedBuilder(
                  animation: _threadListController,
                  builder: (context, _) => CupertinoButton(
                      padding: EdgeInsets.zero,
                      onPressed: _threadListController.items.isEmpty
                          ? null
                          : () => thread_actions.unsaveAllSavedThreads(context,
                              onMutate: _threadListController.update),
                      child: const Icon(CupertinoIcons.delete))),
              Builder(
                  builder: (context) => CupertinoButton(
                      padding: EdgeInsets.zero,
                      child: Icon(Settings.reverseSavedThreadsSortingSetting
                              .watch(context)
                          ? CupertinoIcons.sort_up
                          : CupertinoIcons.sort_down),
                      onPressed: () async {
                        final before = (
                          Settings.instance.savedThreadsSortingMethod,
                          Settings.instance.reverseSavedThreadsSorting
                        );
            ...
            icon: AnimatedBuilder(animation: threadStateBoxesAnimation, builder: (context, _) => CachingBuilder(value: Persistence.sharedThreadStateBox.values.any((s) => s.savedTime != null), builder: (value) => Builder(builder: (context) => Icon(value ? CupertinoIcons.tray_full_fill : CupertinoIcons.tray_full, color: ChanceTheme.primaryColorOf(context))))),
            masterBuilder: (context, selectedThread, threadSetter) {
              ...
              return RefreshableList<(PersistentThreadState, Thread)>(
                aboveFooter: ValueListenableBuilder(
                    ...
                    builder: (context, list, _) => MissingThreadsControls(
                        ...
                        onFixAbandonedForThreads: (threadsToDelete) async {
                          for (final thread in threadsToDelete) {
                            await thread.imageboard.persistence
                                .getThreadStateIfExists(thread.item)
                                ?.delete();
                          }
                        })),
                useFiltersFromContext: false,
                ...
                listUpdater: (options) async {
                  final loader = _savedThreadsLoader =
                      switch (settings.savedThreadsSortingMethod) {
                    ThreadSortingMethod.alphabeticByTitle =>
                      _SavedThreadsByTitleLoader(),
                    ThreadSortingMethod.savedTime =>
                      _SavedThreadsBySavedTimeLoader(),
                    ThreadSortingMethod.lastPostTime =>
                      _SavedThreadsByLastPostTimeLoader(),
                    ThreadSortingMethod.threadPostTime =>
                      _SavedThreadsByThreadPostTimeLoader(),
                    ThreadSortingMethod other => throw UnsupportedError(
                        'Not supported method to sort saved threads: $other')
                  };
                  final missing = await loader
                      .initialize(settings.reverseSavedThreadsSorting);
                ...
                itemBuilder: (itemContext, pair, options) {
                  final state = pair.$1;
                  final isSelected = selectedThread(
                      itemContext,
                      state.imageboard!.scope(
                          ThreadOrPostIdentifier.thread(state.identifier)));
                  final openInNewTabZone = context.read<OpenInNewTabZone?>();
                  return ImageboardScope(
                          ...
                          actions: [
                            if (openInNewTabZone != null)
                              ContextMenuAction(
                                  ...
                                  onPressed: () {
                                    openInNewTabZone.onWantOpenThreadInNewTab(
                                        state.imageboardKey, state.identifier);
                                  }),
                            ContextMenuAction(
                          ...
                          child: GestureDetector(
                              ...
                              child: Builder(
                                  builder: (context) => ThreadRow(
                                      ...
                                      onThumbnailLoadError:
                                          (error, stackTrace) {
                                        state.imageboard?.threadWatcher
                                            .fixBrokenThread(state.identifier);
                                      },
                                      semanticParentIds: const [-12],
                                      onThumbnailTap: (initialAttachment) {
                                        final attachments = _threadListController
                                            .items
                                            .expand((item) => item
                                                .item.$2.attachments
                                                .map((attachment) =>
                                                    TaggedAttachment(
                                                        imageboard: item.item.$1
                                                            .imageboard!,
                                        ...
                                        showGalleryPretagged(
                                            ...
                                            onThreadSelected: (t) =>
                                                threadSetter(t.imageboard.scope(t
                                                    .item
                                                    .identifier
                                                    .threadOrPostIdentifier)),
                                            initialAttachment:
                                                initialAttachment,
                                            onChange: (attachment) {
                                              _threadListController.animateTo(
                                                  (p) =>
                                                      p.$1.imageboard ==
                                                          attachment
                                                              .imageboard &&
                              ...
                              onTap: () => threadSetter(state.imageboard!.scope(
                                  state.identifier.threadOrPostIdentifier)))));
                },
        ...
        paneCreator3: () => MultiMasterPane<PostThreadCombo>(
            navigationBar: const AdaptiveBar(title: Text('Your Posts')),
            icon: Builder(builder: (context) => Icon(CupertinoIcons.pencil, color: ChanceTheme.primaryColorOf(context))),
            masterBuilder: (context, selected, setter) {
              return RefreshableList<PostThreadCombo>(
                  ...
                  includeImageboardKeyAndBoardInSearchString: true,
                  filterableAdapter: (t) =>
                      (t.imageboard.key, t.post.isThread ? t.thread : t.post),
                  controller: _yourPostsListController,
                  listUpdater: (options) async {
                    _yourPostsLists = {};
                    final missing = <ImageboardScoped<ThreadIdentifier>>[];
                    for (final state
                        in Persistence.sharedThreadStateBox.values) {
                      final imageboard = state.imageboard;
                      ...
                      final l = _yourPostsLists
                          .putIfAbsent((imageboard, state.board), () => []);
                  ...
                  sortMethods: [
                    (a, b) =>
                        b.post.time.toLocal().compareTo(a.post.time.toLocal())
                  ],
                          ...
                          child: Builder(
                              builder: (context) => PostRow(
                                  ...
                                  onThumbnailLoadError: (e, st) async {
                                    await item.imageboard.threadWatcher
                                        .fixBrokenThread(
                                            item.thread.identifier);
                                  },
                                  onThumbnailTap: (initialAttachment) {
                                    final attachments = _yourPostsListController
                                        .items
                                        .expand((item) => item
                                    ...
                                    showGalleryPretagged(
                                        ...
                                        posts: {
                                          for (final state
                                              in _yourPostsListController.items)
                                            for (final attachment
                                                in state.item.post.attachments)
                                              attachment: state.item.imageboard
                                                  .scope(state.item.post)
                                        },
                                        initialAttachment: initialAttachment,
                                        onChange: (attachment) {
                                          _yourPostsListController.animateTo(
                                              (p) =>
                                                  p.imageboard ==
                                                      attachment.imageboard &&
                                                  p.post.board.toLowerCase() ==
                                                      attachment
                                                          .attachment.board
                                                          .toLowerCase() &&
                                                  p.post.id ==
        ...
        paneCreator4: () => MultiMasterPane<ImageboardScoped<SavedPost>>(
            navigationBar: AdaptiveBar(title: const Text('Saved Posts'), actions: [
              AnimatedBuilder(
                  animation: _postListController,
                  builder: (context, _) => CupertinoButton(
                      padding: EdgeInsets.zero,
                      onPressed: _postListController.items.isEmpty
                          ? null
                          : () => thread_actions.unsaveAllSavedPosts(context,
                              onMutate: _postListController.update),
                      child: const Icon(CupertinoIcons.delete))),
              Builder(
                  builder: (context) => CupertinoButton(
                      padding: EdgeInsets.zero,
            ...
            icon: AnimatedBuilder(animation: savedPostsNotifiersAnimation, builder: (context, _) => CachingBuilder(value: ImageboardRegistry.instance.imageboardsIncludingDev.any((i) => i.persistence.savedPosts.isNotEmpty), builder: (value) => Builder(builder: (context) => Icon(value ? CupertinoIcons.reply_thick_solid : CupertinoIcons.reply_all, color: ChanceTheme.primaryColorOf(context))))),
            masterBuilder: (context, selected, setter) {
              ...
              return RefreshableList<ImageboardScoped<(SavedPost, Thread)>>(
                  aboveFooter: ValueListenableBuilder(
                      ...
                      builder: (context, list, _) => MissingThreadsControls(
                            ...
                            onFixAbandonedForThreads: (threadsToDelete) async {
                              for (final thread in threadsToDelete) {
                                final postToDelete = thread
                                    .imageboard.persistence.savedPosts.values
                                    .tryFirstWhere((p) =>
                                        p.post.threadIdentifier == thread.item);
                                if (postToDelete != null) {
                                  thread.imageboard.persistence
                                      .unsavePost(postToDelete.post);
                                }
                              }
                            },
                          )),
                  ...
                  filterableAdapter: (t) => (
                        t.imageboard.key,
                        t.item.$1.post.isThread ? t.item.$2 : t.item.$1.post
                      ),
                  controller: _postListController,
                  listUpdater: (options) async {
                    final savedPosts = ImageboardRegistry
                        .instance.imageboardsIncludingDev
                        .expand(
                            (i) => i.persistence.savedPosts.values.map(i.scope))
                        .toList();
                    final pairs = await Future.wait(savedPosts.map((s) async {
                      return (
                        s,
                        await s.imageboard.persistence
                            .getThreadStateIfExists(
                                s.item.post.threadIdentifier)
                            ?.getThread()
                      );
                    }));
                    ...
                    for (final (p, t) in pairs) {
                      if (t != null) {
                        ...
                        missing.add(
                            p.imageboard.scope(p.item.post.threadIdentifier));
                      }
                    }
                  ...
                  itemBuilder: (context, savedPost, options) {
                    final threadState = savedPost.imageboard.persistence
                        .getThreadStateIfExists(
                            savedPost.item.$1.post.threadIdentifier);
                    return ImageboardScope(
                        ...
                        child: ChangeNotifierProvider<PostSpanZoneData>(
                            create: (context) => PostSpanRootZoneData(
                                ...
                                thread:
                                    threadState?.thread ?? savedPost.item.$2,
                                semanticRootIds: [-2],
                                style: PostSpanZoneStyle.linear),
                            child: Builder(
                                builder: (context) => PostRow(
                                    ...
                                    isSelected: selected(
                                        context,
                                        savedPost.imageboard
                                            .scope(savedPost.item.$1)),
                                    onTap: () => setter(savedPost.imageboard
                                        .scope(savedPost.item.$1)),
                                    showBoardName: true,
                                    showSiteIcon: true,
                                    hideThumbnails: options.hideThumbnails,
                                    ...
                                    onThumbnailLoadError: (e, st) async {
                                      final firstThread = threadState?.thread ??
                                          savedPost.item.$2;
                                      await savedPost.imageboard.threadWatcher
                                          .fixBrokenThread(savedPost
                                              .item.$1.post.threadIdentifier);
                                      if (firstThread != threadState!.thread ||
                                          threadState.thread?.archiveName !=
                                              null) {
                                        savedPost.item.$1.post = threadState
                                            .thread!.posts
                                            .firstWhere((p) =>
                                                p.id ==
                                                savedPost.item.$1.post.id);
                                        savedPost.imageboard.persistence
                                            .didUpdateSavedPost();
                                      }
                                    },
                                    onThumbnailTap: (initialAttachment) {
                                      final attachments = _postListController
                                          .items
                                          .expand((item) => item
                                              .item.item.$1.post.attachments
                                              .map((attachment) =>
                                                  TaggedAttachment(
                                                      imageboard:
                                                          item.item.imageboard,
                                      ...
                                      showGalleryPretagged(
                                          ...
                                          posts: {
                                            for (final state
                                              ...
                                              for (final attachment in state
                                                  ...
                                                  .attachments)
                                                attachment:
                                                    state.item.imageboard.scope(
                                                        state.item.item.$1.post)
                                          },
                                          initialAttachment: initialAttachment,
                                          onChange: (attachment) {
                                            _postListController.animateTo((p) =>
                                                p.imageboard ==
                                                    attachment.imageboard &&
                                                p.item.$1.post.board
                                                        .toLowerCase() ==
                                          ...
                                          heroOtherEndIsBoxFitCover: Settings
                                              .instance.squareThumbnails);
                                    }))));
                  },
                  filterHint: 'Search saved posts');
            },
            detailBuilder: (selected, setter, poppedOut) => BuiltDetailPane(widget: selected == null ? _placeholder('Select a post') : ImageboardScope(imageboardKey: selected.imageboard.key, child: ThreadPage(thread: selected.item.post.threadIdentifier, initialPostId: selected.item.post.id, boardSemanticId: -2)), pageRouteBuilder: fullWidthCupertinoPageRouteBuilder)),
        paneCreator5: () => MultiMasterPane<ImageboardScoped<SavedAttachment>>(
            navigationBar: AdaptiveBar(title: const Text('Saved Attachments'), actions: [
              Builder(
                  builder: (context) => AnimatedBuilder(
                      animation: TickerMode.of(context)
                          ? savedAttachmentsNotifiersAnimation
                          : const AlwaysStoppedAnimation(null),
                      builder: (context, _) => CupertinoButton(
                          padding: EdgeInsets.zero,
                          onPressed: ImageboardRegistry.instance.imageboards
                                  .any((i) =>
                                      i.persistence.savedAttachments.isNotEmpty)
                              ? () async {
                                  final toDelete = ImageboardRegistry
                                      .instance.imageboards
                                      .expand((i) => i
                                          .persistence.savedAttachments.values
                                          .map(i.scope))
                                      .toList();
                                  final ok = await showAdaptiveDialog<bool>(
                                      ...
                                      builder: (context) => AdaptiveAlertDialog(
                                              ...
                                              content: Text(
                                                  'All ${describeCount(toDelete.length, 'saved attachment')} will be removed.'),
                                              actions: [
                                  ...
                                  final imageboards =
                                      toDelete.map((i) => i.imageboard).toSet();
                                  for (final item in toDelete) {
                                    item.imageboard.persistence.savedAttachments
                                        .remove(item.item.attachment.globalId);
                                  }
                                  for (final imageboard in imageboards) {
                                    imageboard
                                        .persistence.savedAttachmentsListenable
                                        .didUpdate();
                                  ...
                                  showUndoToast(
                                      context: context,
                                      message:
                                          'Deleted ${describeCount(toDelete.length, 'attachment')}',
                                      onUndo: () {
                                        // Restore all the objects
                                        for (final item in toDelete) {
                                          item.imageboard.persistence
                                                  .savedAttachments[
            ...
            icon: Builder(builder: (context) => Icon(Adaptive.icons.photo, color: ChanceTheme.primaryColorOf(context))),
            masterBuilder: (context, selected, setter) => RefreshableList<ImageboardScoped<SavedAttachment>>(
                ...
                listUpdater: (options) async {
                  ...
                  for (final imageboard
                      in ImageboardRegistry.instance.imageboardsIncludingDev) {
                    for (final entry in imageboard
                        .persistence.savedAttachments.entries
                        .toList()) {
                      if (!imageboard.persistence.savedAttachments
                  ...
                  list.sort(
                      (a, b) => b.item.savedTime.compareTo(a.item.savedTime));
                    ...
                    builder: (context, missing, _) => MissingAttachmentsControls(
                        ...
                        onFixAbandonedForAttachments: (attachments) async {
                          for (final a in attachments) {
                            a.imageboard.persistence
                                .deleteSavedAttachment(a.item.attachment);
                          }
                        })),
                ...
                itemBuilder: (context, item, options) => Builder(builder: (context) {
                      makeController() => AttachmentViewerController(
                          attachment: item.item.attachment,
                          context: context,
                          imageboard: item.imageboard,
                          isDownloaded: _downloadedAttachments
                              .contains(item.item.attachment),
                                  ...
                                  child: Container(
                                      decoration: BoxDecoration(
                                          ...
                                          borderRadius: const BorderRadius.all(
                                              Radius.circular(4)),
                                          border: Border.all(
                                              color: selected(context, item)
                                                  ? ChanceTheme.primaryColorOf(
                                                      context)
                                                  : Colors.transparent,
                                              width: 2)),
                                      ...
                                      child: Hero(
                                          tag: TaggedAttachment(
                                              ...
                                              semanticParentIds: [
                                                -5,
                                                imageboardIds.putIfAbsent(
                                                    item.imageboard.key,
                                                    () => imageboardIds.length)
                                              ],
                                              imageboard: item.imageboard,
                                              postId: 0),
                                          ...
                                          createRectTween:
                                            ...
                                            if (startRect != null &&
                                              ...
                                              if (item.item.attachment.type ==
                                                ...
                                                final rootPadding = MediaQueryData
                                                            .fromView(View.of(
                                                                context))
                                                        .padding -
                                                    sumAdditionalSafeAreaInsets();
                                                startRect = rootPadding
                                                    .deflateRect(startRect);
                                              }
                                            }
            ...
            detailBuilder: (selectedValue, setter, poppedOut) {
              ...
              if (selectedValue == null) {
                ...
                final thisImageboardId = imageboardIds.putIfAbsent(
                    selectedValue.imageboard.key, () => imageboardIds.length);
                final attachment = TaggedAttachment(
                    attachment: selectedValue.item.attachment,
                    semanticParentIds: poppedOut
                        ? [-5, thisImageboardId]
                        : [-6, thisImageboardId],
                ...
                child = ImageboardScope(
                    ...
                    child: Builder(
                        builder: (innerContext) => GalleryPage(
                              ...
                              attachments:
                                  _savedAttachmentsController.items.map((l) {
                                final thisImageboardId =
                                    imageboardIds.putIfAbsent(
                                        l.item.imageboard.key,
                                        () => imageboardIds.length);
                              ...
                              overrideSources: {
                                for (final l
                                    in _savedAttachmentsController.items)
                                  l.item.item.attachment: l.item.item.file.uri
                              },
                              onChange: (a) {
                                final originalL = _savedAttachmentsController
                                    .items
                                    .tryFirstWhere((l) =>
                                        l.item.item.attachment == a.attachment)
                                    ?.item;
                                _savedAttachmentsController
                                    .animateToIfOffscreen(
                                        (l) =>
                                            l.item.attachment == a.attachment,
                                        alignment: 0.5);
                                widget.masterDetailKey.currentState?.setValue5(
                                    originalL,
                              ...
                              additionalContextMenuActionsBuilder:
                                ...
                                _makeFindSavedAttachmentInThreadAction(
                                    ...
                                    imageboard: ImageboardRegistry.instance
                                        .getImageboard(imageboardIds.entries
                                            .tryFirstWhere((e) =>
                                                e.value ==
                                                attachment
                                                    .semanticParentIds.last)
                                            ?.key))
                              ],
                            )));
              }
              return BuiltDetailPane(
                  widget: child, pageRouteBuilder: transparentPageRouteBuilder);
            }));
  }
...
class LocalWatcherControls extends StatelessWidget {
  const LocalWatcherControls({Key? key}) : super(key: key);
            ...
            builder: (context, _) => Row(children: [
                  ...
                  Expanded(
                      child: Column(
                        ...
                        if (w.updatingNow != null ||
                            (w.nextUpdate != null && w.lastUpdate != null))
                          ClipRRect(
                              borderRadius:
                              ...
                              child: TimedRebuilder(
                                  ...
                                  function: () {
                                    ...
                                    return w.updatingNow != null
                                        ? null
                                        : now
                                                .difference(w.lastUpdate!)
                                                .inSeconds /
                                            w.nextUpdate!
                                                .difference(w.lastUpdate!)
                                                .inSeconds;
                                  ...
                                  builder: (context, value) {
                                    return LinearProgressIndicator(
                                        value: w.updatingNow != null
                                            ? null
                                            : value,
                                        color:
                                            ChanceTheme.primaryColorOf(context)
...
class _MissingControls<T extends Object> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CupertinoButton(
        ...
        onPressed: () {
          modalLoad(context,
              'Fetching ${describeCount(missing.length, 'missing $singularNoun')}',
              (controller) async {
            ...
            if (failed.length == list.length && context.mounted) {
              ...
              final clearFailed = (await showAdaptiveDialog<bool>(
                      ...
                      builder: (context) => AdaptiveAlertDialog(
                              title: Text(
                                  '${describeCount(failed.length, 'missing $singularNoun')} not found'),
                              content: Text(
                                  '''Some ${singularNoun}s could not be re-downloaded.
...
${failed.map((t) => '${t.imageboard.site.name}: ${t.item}').join('\n')}'''),
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Flexible(
              child: Text(
                  describeCount(missing.length, 'missing $singularNoun'),
                  style: TextStyle(
                      color: errorColor,
...
class MissingThreadsControls extends StatelessWidget {
  ...
  final VoidCallback afterFix;
  final Future<void> Function(List<ImageboardScoped<ThreadIdentifier>>)
      onFixAbandonedForThreads;
  ...
  Widget build(BuildContext context) => _MissingControls(
        ...
        fixer: (thread) async {
          try {
            ...
            if (_hasLocalDownload(
                thread.imageboard.persistence.getThreadState(thread.item))) {
            ...
            if (newThread == null) {
              try {
                ...
                try {
                  newThread = await thread.imageboard.site.getThreadFromArchive(
                      thread.item,
                      priority: RequestPriority.interactive);
                } catch (e) {
                  if (context.mounted) {
                    showToast(
                        ...
                        message:
                            'Failed to get ${thread.item} from archive: ${e.toStringDio()}');
                  }
                }
              } catch (e) {
                if (context.mounted) {
                  showToast(
                      ...
                      icon: CupertinoIcons.exclamationmark_triangle,
                      message:
                          'Failed to get ${thread.item}: ${e.toStringDio()}');
                }
              }
            }
            if (newThread != null) {
...
class MissingAttachmentsControls extends StatelessWidget {
  ...
  final VoidCallback afterFix;
  final Future<void> Function(List<ImageboardScoped<SavedAttachment>>)
      onFixAbandonedForAttachments;
  ...
  Widget build(BuildContext context) => _MissingControls(
        ...
        fixer: (attachment) async {
          try {
            try {
              // Try naive URL
              await attachment.imageboard.site.client.download(
                  attachment.item.attachment.url, attachment.item.file.path,
                  options: Options(headers: {
                    ...attachment.imageboard.site
                        .getHeaders(Uri.parse(attachment.item.attachment.url)),
                    if (attachment.item.attachment.useRandomUseragent)
            ...
            if (threadId != null) {
              final threadIdentifier =
                  ThreadIdentifier(attachment.item.attachment.board, threadId);
              try {
                // Get the archived thread
                final redirectUrls = <String, String>{};
                final archivedThread = await attachment.imageboard.site
                    .getThreadFromArchive(threadIdentifier,
                        priority: RequestPriority.interactive,
                        customValidator: (thread) async {
                  final found = _findMatchingAttachment(
                          thread, attachment.item.attachment)
                      ?.attachment;
                  if (found == null) {
                    throw Exception(
                        'Attachment not found in ${thread.archiveName}');
                  }
                  if (found.url == attachment.item.attachment.url) {
                    throw Exception(
                        'Attachment not really archived on ${thread.archiveName}');
                  }
                  final response = await attachment.imageboard.site.client.head(
                      found.url,
                      options: Options(headers: {
                        ...attachment.imageboard.site
                            .getHeaders(Uri.parse(found.url)),
                        if (found.useRandomUseragent)
                          'user-agent': makeRandomUserAgent()
                ...
                final found = _findMatchingAttachment(
                        archivedThread, attachment.item.attachment)
                    ?.attachment;
                if (found != null) {
                  await attachment.imageboard.site.client.download(
                      redirectUrls[found.url] ?? found.url,
                ...
                if (context.mounted) {
                  showToast(
                      ...
                      icon: CupertinoIcons.exclamationmark_triangle,
                      message:
                          'Failed to get $threadIdentifier from archive: ${e.toStringDio()}');
                }
              }
            }
          } catch (e, st) {
