...
class _ThreadHidingDialogState extends State<_ThreadHidingDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.thread.posts_.tryFirst?.name case String name
        when name.isNotEmpty && name != imageboard.site.defaultUsername) {
      nameFilter = CustomFilter(
          pattern: RegExp('^${RegExp.escape(name)}\$', caseSensitive: false),
          label: 'Name "$name"',
          patternFields: ['name'],
          sites: {imageboard.key}).toStringConfiguration();
    }
    if (widget.thread.posts_.tryFirst?.trip case String trip
        when trip.isNotEmpty) {
    ...
    if (widget.thread.posts_.tryFirst?.flag case final flag?
        when flag.name.isNotEmpty) {
    ...
    final general = newGeneralPattern
        .firstMatch(
            '${widget.thread.title} ${widget.thread.posts_.tryFirst?.name} ${widget.thread.posts_.tryFirst?.text}')
        ?.group(0)
        ?.toLowerCase();
    if (general != null) {
    ...
    return AdaptiveAlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        AdaptiveChoiceControl<NullSafeOptional>(
          ...
          groupValue: imageboard.persistence.browserState
              .getThreadHiding(widget.thread.identifier)
              .value,
          onValueChanged: (newState) {
            imageboard.persistence.browserState
                .setThreadHiding(widget.thread.identifier, newState.value);
        ...
        Padding(
            ...
            child:
              ...
              Row(children: [
                const Expanded(
                    child: Text('Hide image', style: TextStyle(fontSize: 17))),
                Checkbox.adaptive(
                  value: (widget.isThumbnailHiddenByFilter ||
                          imageboard.persistence.browserState
                              .getThreadImageHiding(
                                  widget.thread.identifier)) &&
                      !imageboard.persistence.browserState
                          .getThreadImageShowOverride(widget.thread.identifier),
                  onChanged: (value) {
                    if (value!) {
                      imageboard.persistence.browserState
                          .setThreadImageHiding(widget.thread.identifier, true);
                      imageboard.persistence.browserState
                          .setThreadImageShowOverride(
                              widget.thread.identifier, false);
                    } else {
                      imageboard.persistence.browserState.setThreadImageHiding(
                          widget.thread.identifier, false);
                      if (widget.isThumbnailHiddenByFilter) {
                        imageboard.persistence.browserState
                            .setThreadImageShowOverride(
                                widget.thread.identifier, true);
                      } else {
                        imageboard.persistence.browserState
                            .setThreadImageShowOverride(
                                widget.thread.identifier, false);
        ...
        if (widget.thread.attachments.any((a) => a.md5.isNotEmpty))
          if (!Settings.applyImageFilterToThreadsSetting.watch(context)) ...[
            ...
            for (final attachment
                in widget.thread.attachments.where((a) => a.md5.isNotEmpty))
              Padding(
                  ...
                  child: Row(
                      ...
                      children: [
                        AttachmentThumbnail(
                            ...
                            mayObscure: false),
                        Checkbox.adaptive(
                            value: context.select<Settings, bool>(
                                (p) => p.isMD5Hidden(attachment.md5)),
                            onChanged: attachment.md5.isEmpty
                                ? null
                                : (value) {
                                    if (value!) {
                                      Settings.instance
                                          .hideByMD5(attachment.md5);
                                    } else {
                                      Settings.instance
                                          .unHideByMD5(attachment.md5);
                                    }
...
class BoardPageState extends State<BoardPage> {
  ...
  final _boardsPullTabKey =
      GlobalKey(debugLabel: '_BoardPageState._boardsPullTabKey');
  final _threadPullTabHandlerKey =
      GlobalKey<_BoardPageThreadPullTabHandlerState>(
          debugLabel: '_BoardPageState._threadPullTabHandlerKey');
  int _page = 1;
  DateTime? _lastCatalogUpdateTime;
  bool _searching = false;
  bool _skipNextIndicatorSwipe = false;

  CatalogVariant? get _defaultBoardVariant => context
      .read<Persistence?>()
      ?.browserState
      .catalogVariants[board?.boardKey];
  CatalogVariant? get _defaultGlobalVariant =>
      context.read<ImageboardSite?>()?.defaultCatalogVariant;
  ...
  void initState() {
    ...
    _searching =
        (widget.initialSearch ?? widget.tab?.initialSearch)?.isNotEmpty ??
            false;
  }

  Future<void> _selectBoard() async {
    final newBoard = await Navigator.of(context)
        .push<ImageboardScoped<ImageboardBoard>>(TransparentRoute(
  ...
  void swapBoard(ImageboardScoped<ImageboardBoard> newBoard) {
    ...
    setState(() {
      ...
      _variant = null;
      if (!newBoard.supportsPosting &&
          (_replyBoxKey.currentState?.show ?? false)) {
  ...
  void _onThreadSelected(ThreadIdentifier identifier) {
    final thread = _listController.items
        .tryFirstWhere((t) => t.item.identifier == identifier)
        ?.item;
    if (thread != null) {
      ...
      context.read<ImageboardSite?>()?.ensureCatalogCached(
          thread,
          _listController.state?.lastUpdateTime ??
  ...
  Future<(CatalogVariant?, _ThreadSortingMethodScope)?> _variantDetailsMenu(
      ...
      showAdaptiveModalPopup<(CatalogVariant?, _ThreadSortingMethodScope)>(
          ...
          builder: (context) => AdaptiveActionSheet(
              ...
              actions: [
                if (context.read<ImageboardSite>().supportsMultipleBoards)
                  AdaptiveActionSheetAction(
                      isSelected: _defaultBoardVariant == variant,
                      trailing: _defaultBoardVariant == variant
                          ? AdaptiveIconButton(
                              minimumSize: Size.zero,
                              icon: const SizedBox(
                                  width: 40, child: Icon(CupertinoIcons.xmark)),
                              onPressed: () => Navigator.pop(context, const (
                                    null,
                                    _ThreadSortingMethodScope.board
                                  )))
                          : null,
                      child: Text('Set as default for /${board?.name}/',
                          textAlign: TextAlign.left),
                      onPressed: () {
                        if (_defaultBoardVariant == variant) return;
                        Navigator.pop(context,
                            (variant, _ThreadSortingMethodScope.board));
                      }),
                AdaptiveActionSheetAction(
                    isSelected: _defaultGlobalVariant == variant,
                    child: const Text('Set as global default',
                        textAlign: TextAlign.left),
                    onPressed: () {
                      if (_defaultGlobalVariant == variant) return;
                      Navigator.pop(
                          context, (variant, _ThreadSortingMethodScope.global));
                    }),
                ...others.map((other) => _buildVariantDetails(
                    context: context,
                    v: CatalogVariantGroup(name: other.name, variants: [other]),
    ...
    required CatalogVariant currentVariant,
      ...
      AdaptiveActionSheetAction(
          ...
          trailing: Row(mainAxisSize: MainAxisSize.min, children: [
            if (v.variants.contains(_variant))
              AdaptiveIconButton(
                  minimumSize: Size.zero,
                  icon: const SizedBox(
                      width: 40, child: Icon(CupertinoIcons.xmark)),
                  onPressed: () => Navigator.pop(
                      context, const (null, _ThreadSortingMethodScope.tab))),
            if ((v.hasPrimary || v.variants.length == 1) &&
                !v.variants.first.temporary)
              AdaptiveIconButton(
                  minimumSize: Size.zero,
                  icon: const SizedBox(
                      width: 40, child: Icon(CupertinoIcons.ellipsis)),
                  onPressed: () async {
            ...
            else if (v.variants.length > 1)
              const SizedBox(
                  width: 40, child: Icon(CupertinoIcons.chevron_right))
          ]),
          child: Row(children: [
            SizedBox(
                width: 40,
                child: Center(
                    child: Icon(v.variants.tryFirst?.icon ??
                        ((v.variants.tryFirst?.reverseAfterSorting ?? false)
                            ? CupertinoIcons.sort_up
                            : CupertinoIcons.sort_down)))),
            Expanded(child: Text(v.name, textAlign: TextAlign.left))
          ]),
          onPressed: () async {
            if (((v.variants.length == 1 || v.hasPrimary) &&
                v.variants.first == currentVariant)) {
              return;
            }
            if (v.hasPrimary || v.variants.length == 1) {
              Navigator.pop(
                  context, (v.variants.first, _ThreadSortingMethodScope.tab));
            } else {
              final choice = await showAdaptiveModalPopup<
                      (CatalogVariant?, _ThreadSortingMethodScope)>(
                  context: context,
                  useRootNavigator: false,
                  builder: (context) => AdaptiveActionSheet(
                      title: Text(v.name),
                      actions: v.variants
                          .map((subvariant) => AdaptiveActionSheetAction(
                              child: Row(children: [
                                SizedBox(
                                    width: 40,
                                    child: Center(
                                      child: Icon(subvariant.icon ??
                                          (subvariant.reverseAfterSorting
                                              ? CupertinoIcons.sort_up
                                              : CupertinoIcons.sort_down)),
                                    )),
                                Expanded(
                                    child: Text(subvariant.name,
                                        style: TextStyle(
                                            fontWeight:
                                                subvariant == currentVariant
                                                    ? FontWeight.bold
                                                    : null,
                                            fontVariations:
                                                subvariant == currentVariant
  ...
  void _showGalleryFromNextImage({bool initiallyShowGrid = false}) {
    ...
    if (board != null && Settings.instance.showImages(context, board.name)) {
      final nextThreadWithImage = _listController.items
          .skip(max(0, _listController.firstVisibleIndex))
          .firstWhere((t) => t.item.attachments.isNotEmpty, orElse: () {
        return _listController.items
            .firstWhere((t) => t.item.attachments.isNotEmpty);
      });
      final imageboard = context.read<Imageboard>();
      final attachments = _listController.items
          .expand((item) => item.item.attachments.map((attachment) =>
              TaggedAttachment(
      ...
      showGalleryPretagged(
          ...
          initialAttachment:
              attachments.firstWhere((a) => nextThreadWithImage.id == a.postId),
          onChange: (attachment) {
            if (_listController.state?.searching ?? false) {
              return;
            }
            _listController.animateToIfOffscreen(
                (p) => p.id == attachment.postId,
                alignment: 0.5);
          },
                  ...
                  child: Container(
                      ...
                      child: ReplyBox(
                          ...
                          onReplyPosted: (board, receipt) async {
                            ...
                            final newThread =
                                ThreadIdentifier(board, receipt.id);
  ...
  Widget build(BuildContext context) {
    ...
    final variant = _variant ??
        (_defaultBoardVariant ?? _defaultGlobalVariant) ??
        CatalogVariant.unsorted;
    final openInNewTabZone = context.read<OpenInNewTabZone?>();
    final useCatalogGrid =
        persistence?.browserState.useCatalogGridPerBoard[board?.boardKey] ??
            persistence?.browserState.useCatalogGrid ??
            settings.useCatalogGrid;
    Widget itemBuilder(BuildContext context, Thread thread,
        RefreshableListItemOptions options) {
      final isSaved = context.select<Persistence, bool>((p) =>
          p.getThreadStateIfExists(thread.identifier)?.savedTime != null);
      final isYou = context.select<Persistence, bool>((p) =>
          p
              .getThreadStateIfExists(thread.identifier)
              ?.youIds
              .contains(thread.id) ??
          false);
      final watch = context.select<Persistence, ThreadWatch?>(
          (p) => p.getThreadStateIfExists(thread.identifier)?.threadWatch);
      final isThreadHidden = context.select<Persistence, bool?>(
          (p) => p.browserState.getThreadHiding(thread.identifier));
      final isImageHidden =
          context.select<Settings, bool>((p) => p.areMD5sHidden(thread.md5s));
      final isImageHiddenByUser = context.select<Persistence, bool>(
          (p) => p.browserState.getThreadImageHiding(thread.identifier));
      final isImageShowOverride = context.select<Persistence, bool>(
          (p) => p.browserState.getThreadImageShowOverride(thread.identifier));
      final isSelected =
          widget.isThreadSelected?.call(context, thread.identifier) ?? false;
          ...
          actions: [
              ...
              ContextMenuAction(
                  ...
                  onPressed: () {
                    openInNewTabZone.onWantOpenThreadInNewTab(
                        imageboard!.key, thread.identifier,
                        activate: false);
                  }),
              ContextMenuAction(
            ...
            if (isSaved)
              ContextMenuAction(
                  ...
                  onPressed: () {
                    final threadState = context
                        .read<Persistence>()
                        .getThreadState(thread.identifier);
              ...
              ContextMenuAction(
                  ...
                  onPressed: () async {
                    ...
                    if (settings.watchThreadAutomaticallyWhenReplying) {
                      if ((site?.supportsPushNotifications ?? false) &&
                          context.mounted) {
                      ...
                      imageboard?.notifications.subscribeToThread(
                          ...
                          localYousOnly:
                              (threadState.threadWatch ?? settings.defaultThreadWatch)
                                      ?.localYousOnly ??
                                  true,
                          pushYousOnly:
                              (threadState.threadWatch ?? settings.defaultThreadWatch)?.pushYousOnly ??
                                  true,
                          foregroundMuted:
                              (threadState.threadWatch ?? settings.defaultThreadWatch)
                                      ?.foregroundMuted ??
                                  false,
                          push: (threadState.threadWatch ?? settings.defaultThreadWatch)?.push ??
                              true,
                          youIds: threadState.freshYouIds(),
                          notifyOnSecondLastPage:
                              (threadState.threadWatch ?? settings.defaultThreadWatch)
                                      ?.notifyOnSecondLastPage ??
                                  false,
                          notifyOnLastPage:
                              (threadState.threadWatch ?? settings.defaultThreadWatch)?.notifyOnLastPage ?? true,
              ...
              ContextMenuAction(
                  ...
                  onPressed: () async {
                    ...
                    imageboard.notifications.subscribeToThread(
                        ...
                        notifyOnSecondLastPage: settings
                                .defaultThreadWatch?.notifyOnSecondLastPage ??
                            false,
                        notifyOnLastPage:
                            settings.defaultThreadWatch?.notifyOnLastPage ??
                                true,
                        notifyOnDead:
                            settings.defaultThreadWatch?.notifyOnDead ?? false);
                    setState(() {});
                    if (context.mounted) {
                      showUndoToast(
                          context: context,
                          ...
                          onUndo: () {
                            imageboard.notifications
                                .unsubscribeFromThread(thread.identifier);
                            setState(() {});
                          });
                    }
                  }),
            if (isThreadHidden ?? false)
              ContextMenuAction(
                  ...
                  onPressed: () {
                    ...
                    persistence.browserState
                        .setThreadHiding(thread.identifier, null);
                    showUndoToast(
                        context: context,
                        message: 'Thread unhidden',
                        onUndo: () {
                          persistence.browserState
                              .setThreadHiding(thread.identifier, original);
              ...
              ContextMenuAction(
                  ...
                  onPressed: () {
                    ...
                    final persistence = context.read<Persistence>();
                    persistence.browserState
                        .setThreadHiding(thread.identifier, true);
                    showUndoToast(
                        context: context,
            ...
            ContextMenuAction(
                child:
                    isHidden ? const Text('Unhide...') : const Text('Hide...'),
                trailingIcon:
                  ...
                  showAdaptiveDialog(
                      ...
                      builder: (context) => ImageboardScope(
                          ...
                          child: _ThreadHidingDialog(
                              thread: thread,
                              listFilterReason: isThreadHiddenByIdOrMD5s
                                  ? null
                                  : listFilterReason,
                              isThumbnailHiddenByFilter:
                                  options.hideThumbnails)));
                }),
            if (thread.attachments.isNotEmpty)
              ContextMenuAction(
                  child:
                      Text('Copy ${thread.attachments.first.type.noun} link'),
                  trailingIcon: CupertinoIcons.link,
                  onPressed: () async {
                    final which =
                        await whichAttachment(context, thread.attachments);
                    if (which == null) {
                      return;
                    }
                    Clipboard.setData(ClipboardData(text: which.url));
                    if (context.mounted) {
                      showToast(
                          context: context,
                            ...
                            final file = await modalLoad(
                                ...
                                (c) => sharePostsAsImage(
                                    ...
                                    style: const ShareablePostsStyle(
                                        expandPrimaryImage: true, width: 400),
                                    zone: zone));
                            zone.dispose();
                            if (context.mounted) {
                              shareOne(
          ...
          child: GestureDetector(
              onDoubleTap: settings.doubleTapToHideThreads
                  ? () {
                      ...
                      final hiding = persistence.browserState
                          .getThreadHiding(thread.identifier);
                      ...
                      persistence.browserState.setThreadHiding(
                          thread.identifier, !(hiding ?? isHidden));
                      persistence.didUpdateBrowserState();
                      setState(() {});
                      if (context.mounted) {
                        showUndoToast(
                            context: context,
                            message:
                                'Thread ${isHidden ? 'unhidden' : 'hidden'}',
                            onUndo: () {
                              persistence.browserState
                                  .setThreadHiding(thread.identifier, hiding);
                              persistence.didUpdateBrowserState();
                              setState(() {});
                            });
                      }
              ...
              child: ThreadRow(
                  style: useCatalogGrid
                      ...
                      : ThreadRowStyle.row,
                  showLastReplies:
                      !useCatalogGrid && settings.showLastRepliesInCatalog,
                  thread: thread,
                  ...
                  showPageNumber:
                      options.inFilterFooter || variant.sortingMethod != null,
                  hideThumbnails:
                      (options.hideThumbnails || isImageHiddenByUser) &&
                          !isImageShowOverride,
                  onThumbnailTap: (initialAttachment) {
                    final imageboard = context.read<Imageboard>();
                    final attachments = _listController.items
                        .expand((item) => item.item.attachments.map(
                    ...
                    final initialAttachmentInList = attachments.tryFirstWhere(
                        (a) =>
                            a.attachment.id == initialAttachment.attachment.id);
                    showGalleryPretagged(
                        context: context,
                        attachments: initialAttachmentInList == null
                            ? [initialAttachment]
                            : attachments,
                        threads: {
                          for (final thread in _listController.items)
                        ...
                        onThreadSelected: (t) =>
                            _onThreadSelected(t.item.identifier),
                        initialAttachment:
                            initialAttachmentInList ?? initialAttachment,
                        onChange: (attachment) {
                          if (_listController.state?.searching ?? false) {
                            return;
                          }
                              ...
                              (p) =>
                                  p.board.toLowerCase() ==
                                      attachment.attachment.board
                                          .toLowerCase() &&
                                  p.id == attachment.postId,
                              alignment: 0.5);
                        },
                        heroOtherEndIsBoxFitCover: useCatalogGrid
                            ? settings.catalogGridModeCropThumbnails
    ...
    if (imageboard != null) {
      navigationBarBoardName = board != null
          ? imageboard.site.formatBoardName(board!.name)
          : 'Select Board';
    }
    final supportsSearch = imageboard?.site.supportsSearch(board?.name) ??
        const ImageboardSearchMetadata(
            name: '', options: ImageboardSearchOptions());
    final sortMethods = <Comparator<Thread>>[
      if (variant.sortingMethod == ThreadSortingMethod.replyCount)
        ...
        (a, b) {
          ...
          return -1 *
              ((b.replyCount + 1) / bAge).compareTo((a.replyCount + 1) / aAge);
        }
      else if (variant.sortingMethod ==
          ThreadSortingMethod.postsPerMinuteWithNewThreadsAtTop)
        (a, b) {
    ...
    final radius = settings.materialStyle
    ...
    return AdaptiveScaffold(
        ...
        bar: AdaptiveBar(
                ...
                icon: DefaultTextStyle.merge(
                    ...
                    child: Wrap(
                        ...
                        children: [
                          Row(mainAxisSize: MainAxisSize.min, children: [
                            ...
                            if (imageboard != null) ...[
                              if (ImageboardRegistry.instance.count > 1) ...[
                                ImageboardIcon(boardName: board?.name),
                                const Text(' ')
                              ]
                            ]
                          ]),
                          Row(mainAxisSize: MainAxisSize.min, children: [
                            Flexible(
                                child: AutoSizeText(navigationBarBoardName,
                                    minFontSize: 9, maxLines: 1)),
                            if (widget.allowChangingBoard)
                    ...
                    onPressed: () {
                      Navigator.of(context).push(TransparentRoute(
                          builder: (context) => BoardSettingsPage(
                              imageboard: imageboard!, board: board!)));
                    }),
              AdaptiveBarAction(
                  title: 'Sort...',
                  icon: (variant.icon != null &&
                          !variant.temporary &&
                          variant != _defaultGlobalVariant)
                      ? FittedBox(
                          fit: BoxFit.contain,
                          child: SizedBox(
                              width: 40,
                              ...
                              child: Stack(children: [
                                ...
                                Align(
                                    ...
                                    child: Icon(variant.reverseAfterSorting
                                        ? CupertinoIcons.sort_up
                                        : CupertinoIcons.sort_down))
                              ])))
                      : (variant.icon != null && variant.temporary)
                          ? Icon(variant.icon)
                          : Icon(variant.reverseAfterSorting
                              ? CupertinoIcons.sort_up
                              : CupertinoIcons.sort_down),
                  onPressed: () async {
                    final choice = await showAdaptiveModalPopup<
                        ...
                        builder: (context) => AdaptiveActionSheet(
                            ...
                            actions: (site?.catalogVariantGroups ?? [])
                                .map((v) => _buildVariantDetails(
                                    context: context,
                                    v: v,
                    ...
                    if (choice.$1 == null) {
                      if (choice.$2 == _ThreadSortingMethodScope.tab) {
                        _variant = null;
                        widget.tab
                            ?.mutate((tab) => tab.catalogVariant = _variant);
                      } else if (choice.$2 == _ThreadSortingMethodScope.board) {
                        persistence?.browserState.catalogVariants
                            .remove(board?.boardKey);
                      }
                      setState(() {});
                      return;
                    }
                    switch (choice.$2) {
                      ...
                      case _ThreadSortingMethodScope.board:
                        persistence?.browserState
                            .catalogVariants[board!.boardKey] = choice.$1!;
                        persistence?.didUpdateBrowserState();
                        break;
                      case _ThreadSortingMethodScope.tab:
                        final otherwiseDefault =
                            _defaultBoardVariant ?? _defaultGlobalVariant;
            ...
            actions: [
              if (
                  ...
                  (mouseSettings.supportMouse &&
                          !Navigator.of(context).canPop()) ||
                      // Space is generally available
                      !(context
                              .watch<MasterDetailLocation?>()
                              ?.isVeryConstrained ??
                          false))
                AnimatedBuilder(
              ...
              if (!settings.replyButtonAtBottom && supportsPosting)
                NotifyingIcon(
                    ...
                    primaryCount: MappingValueListenable(
                        ...
                        mapper: (o) => o
                            .queuedPostsFor(
                                imageboard?.key ?? '', board?.name ?? '', null)
                    ...
                    secondaryCount: MappingValueListenable(
                        ...
                        mapper: (o) =>
                            o.submittableCount -
                            o
                                .queuedPostsFor(imageboard?.key ?? '',
                                    board?.name ?? '', null)
                                .where((e) => e.state.isSubmittable)
                                .length),
                    icon: AdaptiveIconButton(
                        icon: (_replyBoxKey.currentState?.show ?? false)
                            ? const Icon(CupertinoIcons.pencil_slash)
        ...
        body: PullTab(
            ...
            enabled: settings.openBoardSwitcherSlideGesture &&
                widget.allowChangingBoard,
            child: board == null
                ? Center(
                    child: ErrorMessageCard('No board selected', remedies: {
                ...
                : _BoardPageThreadPullTabHandler(
                    ...
                    child: FilterZone(
                        filter: FilterGroup([
                          context.select<Persistence, Filter>((p) =>
                              p.browserState.getCatalogFilter(board!.boardKey)),
                        ...
                        child: PopScope(
                            ...
                            child: ReplyBoxLayout(
                                    ...
                                    bindings: {
                                      ConditionalShortcut(
                                          ...
                                          condition: () => !(_listController
                                              ...
                                              false)): _showGalleryFromNextImage
                                    },
                                    child: site == null
                                        ? const Center(
                                            child: ErrorMessageCard(
                                                'No imageboard selected'))
                                        : Stack(
                                            fit: StackFit.expand,
                                            children: [
                                                RefreshableList<Thread>(
                                                    ...
                                                    onWantAutowatch: (thread,
                                                        autoWatch) async {
                                                      final imageboard = context
                                                          .read<Imageboard>();
                                                      ...
                                                      imageboard.notifications.subscribeToThread(
                                                          ...
                                                          lastSeenId: thread
                                                              .posts_.last.id,
                                                          localYousOnly: settings
                                                                  .defaultThreadWatch
                                                                  ?.localYousOnly ??
                                                              false,
                                                          pushYousOnly: settings
                                                                  .defaultThreadWatch
                                                                  ?.pushYousOnly ??
                                                              false,
                                                          push: autoWatch.push ??
                                                          ...
                                                          notifyOnSecondLastPage: settings
                                                              ...
                                                              false,
                                                          notifyOnLastPage:
                                                              settings.defaultThreadWatch?.notifyOnLastPage ?? true,
                                                    ...
                                                    reverseSort: variant
                                                        .reverseAfterSorting,
                                                    minCacheExtent: useCatalogGrid
                                                        ? settings
                                                            .catalogGridHeight
                                                        : 0,
                                                    gridDelegate: (useCatalogGrid &&
                                                            !settings
                                                                .useStaggeredCatalogGrid)
                                                        ? SliverGridDelegateWithMaxCrossAxisExtentWithCacheTrickery(
                                                            maxCrossAxisExtent: settings
                                                                .catalogGridWidth,
                                                            childAspectRatio:
                                                    ...
                                                    staggeredGridDelegate:
                                                        (useCatalogGrid &&
                                                                settings.useStaggeredCatalogGrid)
                                                            ? SliverStaggeredGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent: settings.catalogGridWidth)
                                                            : null,
                                                    ...
                                                    listExtender: (after, cancelToken) => site.getMoreCatalog(board!.name, after, variant: variant, priority: RequestPriority.interactive, cancelToken: cancelToken).then((list) async {
                                                          for (final thread
                                                              in list) {
                                                            await thread
                                                                .preinit(
                                                    ...
                                                    filterAlternative: (widget.onWantArchiveSearch == null || !supportsSearch.options.text)
                                                        ? null
                                                        : FilterAlternative(
                                                            name: supportsSearch.name,
                                                            handler: (s) {
                                                              widget.onWantArchiveSearch!(
                                                                  imageboard!
                                                                      .key,
                                                                  board!.name,
                                                                  s);
                                                                ...
                                                                child:
                                                                    AnimatedBuilder(
                                                                        ...
                                                                        builder:
                                                                            (context,
                                                                                _) {
                                                                          if (_listController.state?.originalList ==
                                                                              null) {
                                                                            return const SizedBox.shrink();
                                                                          }
                                                                          final theme =
                                                                              context.watch<SavedTheme>();
                                                                          final primaryColorWithBrightness60 =
                                                                              theme.primaryColorWithBrightness(0.6);
                                                                          final primaryColorWithBrightness80 =
                                                                              theme.primaryColorWithBrightness(0.8);
                                                                          scrollAnimationDuration() => Settings.instance.showAnimations
                                                                              ? const Duration(milliseconds: 200)
                                                                              : const Duration(milliseconds: 1);
                                                                          scrollToTop() => _listController.animateToIndex(
                                                                              0,
                                                                              duration: scrollAnimationDuration());
                                                                          scrollToBottom() => _listController.animateToIndex(
                                                                              _listController.itemsLength - 1,
                                                                              alignment: 1.0,
                                                                              duration: scrollAnimationDuration());
                                                                          final realImageCount = _listController.items.fold<int>(
                                                                              0,
                                                                              (t, a) => t + a.item.attachments.length);
                                                                          return Row(
                                                                              mainAxisSize: MainAxisSize.min,
                                                                              children: [
                                                                                if (settings.showGalleryGridButton && realImageCount > 1) ...[
                                                                                  AdaptiveFilledButton(padding: const EdgeInsets.all(8), minimumSize: Size.zero, color: primaryColorWithBrightness80, onPressed: () => _showGalleryFromNextImage(initiallyShowGrid: true), child: Icon(CupertinoIcons.square_grid_2x2, size: 20, color: theme.backgroundColor, applyTextScaling: true)),
                                                                                  const SizedBox(width: 8),
                                                                                ],
                                                                                if (settings.replyButtonAtBottom && supportsPosting) ...[
                                                                                  ValueListenableBuilder(
                                                                                      valueListenable: Combining2ValueListenable(child1: MappingValueListenable(parent: Outbox.instance, mapper: (o) => o.queuedPostsFor(imageboard?.key ?? '', board?.name ?? '', null).where((e) => e.state.isSubmittable).length), child2: MappingValueListenable(parent: Outbox.instance, mapper: (o) => o.submittableCount - o.queuedPostsFor(imageboard?.key ?? '', board?.name ?? '', null).where((e) => e.state.isSubmittable).length), combine: (v1, v2) => (thisBoard: v1, otherBoards: v2)),
                                                                                      builder: (context, counts, _) => SegmentedWidget(radius: radius, segments: [
                                                                                            if (counts.otherBoards > 0 && !(_replyBoxKey.currentState?.show ?? false)) SegmentedWidgetSegment(color: primaryColorWithBrightness80, child: Text('${counts.otherBoards}', style: TextStyle(color: theme.backgroundColor))),
                                                                                            if (counts.thisBoard > 0 && !(_replyBoxKey.currentState?.show ?? false)) SegmentedWidgetSegment(color: theme.secondaryColor, child: Text('${counts.thisBoard}', style: TextStyle(color: (theme.secondaryColor.computeLuminance() > 0.5) ? Colors.black : Colors.white))),
                                                                                            SegmentedWidgetSegment(color: primaryColorWithBrightness80, onPressed: _onReplyButtonPressed, child: Icon((_replyBoxKey.currentState?.show ?? false) ? CupertinoIcons.pencil_slash : CupertinoIcons.pencil, size: 20, color: theme.backgroundColor, applyTextScaling: true))
                                                                                          ])),
                                                                                  const SizedBox(width: 8),
                                                                                ],
                                                                                GestureDetector(
                                                                                            ...
                                                                                            child: Row(mainAxisSize: MainAxisSize.min, children: [
                                                                                              ...
                                                                                              AnimatedBuilder(
                                                                                                  ...
                                                                                                  builder: (context, _) {
                                                                                                    ...
                                                                                                    return Padding(padding: const EdgeInsets.only(left: 8), child: Text(describeCount(_listController.itemsLength, 'result'), style: TextStyle(color: theme.backgroundColor)));
                                                                                                  })
                                                                                            ])),
                                                                                        SegmentedWidgetSegment(color: theme.primaryColor, onPressed: _listController.state?.closeSearch, child: Text('Close', style: TextStyle(color: theme.backgroundColor)))
                                                                                      ] else ...[
                                                                                        SegmentedWidgetSegment(
                                                                                            ...
                                                                                            child: Row(mainAxisSize: MainAxisSize.min, children: [
                                                                                              if (sortMethods.isEmpty) Icon(CupertinoIcons.doc, color: theme.backgroundColor, size: 19, applyTextScaling: true),
                                                                                              ConstrainedBox(
                                                                                                  constraints: BoxConstraints(minWidth: MediaQuery.textScalerOf(context).scale(19)),
                                                                                                  child: AnimatedBuilder(
                                                                                                      animation: _listController.slowScrolls,
                                                                                                      builder: (context, _) {
                                                                                                        _page = (_listController.firstVisibleItem?.item.currentPage ?? _page);
                                                                                                        return Text((sortMethods.isEmpty ? _page : (_listController.itemsLength - (_listController.lastVisibleIndex + 1))).toString(), textAlign: TextAlign.center, style: TextStyle(color: theme.backgroundColor, fontFeatures: const [FontFeature.tabularFigures()]));
                                                                                                      }))
                                                                                            ]))
                                                                                      ]
                                                                                    ]))
                                ...
                                replyBox: RepaintBoundary(
                                  ...
                                  onDraftChanged: (draft) {
                                    widget.tab
                                        ?.mutate((tab) => tab.draft = draft);
                                  },
                                  onReplyPosted: (board, receipt) async {
                                    if (imageboard
                                            ?.site.supportsPushNotifications ==
                                        true) {
...
class _BoardPageThreadPullTabHandler extends StatefulWidget {
  ...
  const _BoardPageThreadPullTabHandler(
      {required this.child, required this.onPull, super.key});

  @override
  createState() => _BoardPageThreadPullTabHandlerState();
}

class _BoardPageThreadPullTabHandlerState
    extends State<_BoardPageThreadPullTabHandler> {
  ...
  Widget build(BuildContext context) {
    ...
    return PullTab(
        tab: (hint?.currentValue != null ||
                _lastSelectedThread == null ||
                _lastSelectedThread?.$1 != imageboard)
            ? null
            : PullTabTab(
                child: Text(
                    'Re-open /${_lastSelectedThread!.$2.board}/${_lastSelectedThread!.$2.id}'),
