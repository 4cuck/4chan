        ...
        children: [
          ...
          AdaptiveChoiceControl<NullSafeOptional>(
            ...
            onValueChanged: (newState) {
              imageboard.persistence.browserState.setThreadHiding(
              ...
              children: [
                Row(
                  children: [
                    ...
                    Checkbox.adaptive(
                      value:
                          (widget.isThumbnailHiddenByFilter ||
                              imageboard.persistence.browserState
                                  .getThreadImageHiding(
                                    widget.thread.identifier,
                                  )) &&
                          !imageboard.persistence.browserState
                              .getThreadImageShowOverride(
                                widget.thread.identifier,
                              ),
                      onChanged: (value) {
                        if (value!) {
                          imageboard.persistence.browserState
                              .setThreadImageHiding(
                                widget.thread.identifier,
                                true,
                              );
                          imageboard.persistence.browserState
          ...
          if (widget.thread.attachments.any((a) => a.md5.isNotEmpty))
            if (!Settings.applyImageFilterToThreadsSetting.watch(context)) ...[
              ...
              for (final attachment in widget.thread.attachments.where(
                (a) => a.md5.isNotEmpty,
              ))
                Padding(
...
class BoardPageState extends State<BoardPage> {
  ...
  final _boardsPullTabKey = GlobalKey(
    debugLabel: '_BoardPageState._boardsPullTabKey',
  );
  ...
  Future<void> _selectBoard() async {
    final newBoard = await Navigator.of(context)
        .push<ImageboardScoped<ImageboardBoard>>(
          TransparentRoute(
    ...
    trailing: Row(
      ...
      children: [
        if (v.variants.contains(_variant))
          AdaptiveIconButton(
            minimumSize: Size.zero,
            icon: const SizedBox(width: 40, child: Icon(CupertinoIcons.xmark)),
            onPressed: () => Navigator.pop(context, const (
              null,
        ...
        if ((v.hasPrimary || v.variants.length == 1) &&
          ...
          const SizedBox(width: 40, child: Icon(CupertinoIcons.chevron_right)),
      ],
    ),
    child: Row(
      children: [
        ...
        final choice =
            ...
            >(
              ...
              builder: (context) => AdaptiveActionSheet(
                ...
                actions: v.variants
                    .map(
                      (subvariant) => AdaptiveActionSheetAction(
                        child: Row(
                          children: [
                            SizedBox(
                              width: 40,
                            ...
                            Expanded(
                              child: Text(
                                subvariant.name,
                                style: TextStyle(
                                  fontWeight: subvariant == currentVariant
                                      ? FontWeight.bold
                                      : null,
                                  fontVariations: subvariant == currentVariant
  ...
  void _showGalleryFromNextImage({bool initiallyShowGrid = false}) {
    ...
    if (board != null && Settings.instance.showImages(context, board.name)) {
      final nextThreadWithImage = _listController.items
          ...
          .firstWhere(
            (t) => t.item.attachments.isNotEmpty,
            orElse: () {
              return _listController.items.firstWhere(
                (t) => t.item.attachments.isNotEmpty,
              );
            },
          );
      ...
      showGalleryPretagged(
        ...
        initialAttachment: attachments.firstWhere(
          (a) => nextThreadWithImage.id == a.postId,
        ),
        onChange: (attachment) {
  ...
  Future<void> _showAttachmentsPageFromNextImage() async {
    final nextThreadWithImage = _listController.items
        .skip(max(0, _listController.firstVisibleIndex - 1))
        .firstWhere(
  ...
  Widget build(BuildContext context) {
    ...
    Widget itemBuilder(
      ...
      final isSaved = context.select<Persistence, bool>(
        (p) => p.getThreadStateIfExists(thread.identifier)?.savedTime != null,
      );
      final isYou = context.select<Persistence, bool>(
        (p) =>
            p
      ...
      final isImageHidden = context.select<Settings, bool>(
        (p) => p.areMD5sHidden(thread.md5s),
      );
        ...
        actions: [
          ...
          if (isSaved)
            ContextMenuAction(
              ...
              onPressed: () {
                final threadState = context.read<Persistence>().getThreadState(
                  thread.identifier,
                );
            ...
            ContextMenuAction(
              ...
              onPressed: () async {
                ...
                imageboard.notifications.subscribeToThread(
                  ...
                  notifyOnSecondLastPage:
                      settings.defaultThreadWatch?.notifyOnSecondLastPage ??
                      false,
                  notifyOnLastPage:
                ...
                if (context.mounted) {
                  showUndoToast(
                    ...
                    onUndo: () {
                      imageboard.notifications.unsubscribeFromThread(
                        thread.identifier,
                      );
                      setState(() {});
                    },
                ...
                builder: (context) => ImageboardScope(
                  ...
                  child: _ThreadHidingDialog(
                    ...
                    listFilterReason: isThreadHiddenByIdOrMD5s
                        ? null
                        : listFilterReason,
                    isThumbnailHiddenByFilter: options.hideThumbnails,
                  ),
                ),
              );
            },
          ...
          if (thread.attachments.isNotEmpty)
            ContextMenuAction(
              ...
              onPressed: () async {
                final which = await whichAttachment(
                  context,
                  thread.attachments,
                );
                if (which == null) {
        ...
        child: GestureDetector(
          onDoubleTap: settings.doubleTapToHideThreads
              ? () {
                  ...
                  final hiding = persistence.browserState.getThreadHiding(
                    thread.identifier,
                  );
          ...
          child: ThreadRow(
            ...
            onThumbnailTap: (initialAttachment) {
              ...
              final initialAttachmentInList = attachments.tryFirstWhere(
                (a) => a.attachment.id == initialAttachment.attachment.id,
              );
              showGalleryPretagged(
                context: context,
    ...
    final supportsSearch =
        imageboard?.site.supportsSearch(board?.name) ??
    ...
    return AdaptiveScaffold(
      ...
      bar: AdaptiveBar(
            ...
            child: Wrap(
              ...
              children: [
                Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    if (context.read<PersistentBrowserTab?>()?.incognito ??
                        false) ...[
            ...
            onPressed: () async {
              ...
              if (choice.$1 == null) {
                if (choice.$2 == _ThreadSortingMethodScope.tab) {
                  ...
                  persistence?.browserState.catalogVariants.remove(
                    board?.boardKey,
                  );
                }
                setState(() {});
              ...
              switch (choice.$2) {
                ...
                case _ThreadSortingMethodScope.board:
                  persistence?.browserState.catalogVariants[board!.boardKey] =
                      choice.$1!;
                  persistence?.didUpdateBrowserState();
                  break;
        ...
        actions: [
          ...
          (mouseSettings.supportMouse && !Navigator.of(context).canPop()) ||
              ...
              !(context.watch<MasterDetailLocation?>()?.isVeryConstrained ??
                  false))
            AnimatedBuilder(
              animation: _listController,
              builder: (context, _) =>
                  ValueListenableBuilder<
                    ({String id, Future<void> future, CancelToken cancelToken})?
                  >(
      ...
      body: PullTab(
        ...
        enabled:
            settings.openBoardSwitcherSlideGesture && widget.allowChangingBoard,
        child: board == null
            ? Center(
            ...
            : _BoardPageThreadPullTabHandler(
                ...
                child: FilterZone(
                  filter: FilterGroup([
                    context.select<Persistence, Filter>(
                      (p) => p.browserState.getCatalogFilter(board!.boardKey),
                    ),
                      ...
                      body: CallbackShortcuts(
                        bindings: {
                          ConditionalShortcut(
                            ...
                            condition: () =>
                                !(_listController.state?.searchHasFocus ??
                                    false),
                                ...
                                children: [
                                  RefreshableList<Thread>(
                                    initialFilter:
                                        ...
                                        widget.tab?.initialSearch,
                                    onFilterChanged: (newFilter) {
                                      widget.tab?.mutate(
                                        (tab) => tab.initialSearch = newFilter,
                                      );
                                    ...
                                    onWantAutosave: (thread) async {
                                      final persistence = context
                                          .read<Persistence>();
                                      if (persistence
                                              .browserState
                                              .autosavedIds[thread.boardKey]
                                              ?.contains(thread.id) ??
                                          false) {
                                        // Already saw this thread
                                        return;
                                      }
                                      final threadState = persistence
                                          .getThreadState(thread.identifier);
                                      threadState.savedTime = DateTime.now();
                                      if (!threadState.isThreadCached) {
                                        threadState.thread = thread;
                                      }
                                      persistence.browserState.autosavedIds
                                          .putIfAbsent(
                                            thread.boardKey,
                                            () => [],
                                          )
                                    ...
                                    onWantAutowatch: (thread, autoWatch) async {
                                      ...
                                      if (imageboard
                                              .persistence
                                              .browserState
                                              .autowatchedIds[thread.boardKey]
                                              ?.contains(thread.id) ??
                                          false) {
                                        // Already saw this thread
                                        return;
                                    ...
                                    listExtender: (after, cancelToken) => site
                                        .getMoreCatalog(
                                          board!.name,
                                          after,
                                          variant: variant,
                                    ...
                                    disableBottomUpdates:
                                        !(variant.hasPagedCatalog ??
                                            site.hasPagedCatalog),
                                    id: '${site.name} /${board!.name}/${variant.dataId}',
                                    itemBuilder: (context, thread, options) =>
                                        itemBuilder(context, thread, options),
                                    filterHint: 'Search in board',
                                    filterAlternative:
                                        (widget.onWantArchiveSearch == null ||
                                            !supportsSearch.options.text)
                                        ? null
                                        : FilterAlternative(
                                        ...
                                        child: Padding(
                                          ...
                                          child: AnimatedBuilder(
                                            ...
                                            builder: (context, _) {
                                              ...
                                              scrollToBottom() =>
                                                  _listController.animateToIndex(
                                                    _listController
                                                            .itemsLength -
                                                        1,
                                                    alignment: 1.0,
                                                  ...
                                                  if (settings
                                                    ...
                                                    AdaptiveFilledButton(
                                                      ...
                                                      color:
                                                          primaryColorWithBrightness80,
                                                      onPressed: () =>
                                                          _showAttachmentsPageFromNextImage(),
                                                      child: RotatedBox(
                                                        quarterTurns: 1,
                                                        child: Icon(
                                                          CupertinoIcons
                                                                  ...
                                                                  child: AnimatedBuilder(
                                                                    ...
                                                                    builder: (context, _) {
                                                                      ...
                                                                      return Text(
                                                                        (sortMethods.isEmpty
                                                                                ? _page
                                                                                : (_listController.itemsLength -
                                                                                      (_listController.lastVisibleIndex +
                                                                                          1)))
                                                                            .toString(),
                                                                        textAlign:
                                                                            TextAlign.center,
