...
import 'package:chan/pages/quilt_gallery.dart';
...
class _GalleryPageState extends State<GalleryPage> {
  ...
  AttachmentViewerController get currentController =>
      _getController(currentAttachment);
  ...
  void initState() {
    ...
    // Initialize _peers
    if ((widget.threads.values.toSet().length +
            widget.posts.values.toSet().length) >
        1) {
    ...
    if (Settings.instance.autoloadAttachments ||
        Settings.instance.alwaysAutoloadTappedAttachment) {
    ...
    if (maxOffset > 0) {
      ...
      final gridViewHeight =
          ...
          (_thumbnailSize +
              ...
              ((Settings.featureStatusBarWorkaround &&
                      (Persistence.settings.useStatusBarWorkaround ?? false))
                  ? 0
                  : screenTopViewPadding) +
              screenBottomViewPadding);
      final gridViewRowCount =
          (gridViewHeight / (Settings.instance.thumbnailSize * 1.5)).ceil();
  ...
  void _onThumbnailScrollControllerUpdate() {
    ...
    _thumbnailsDesynced |=
        thumbnailScrollController.position.activity is DragScrollActivity;
  ...
  void _onGridViewScrollControllerUpdate() {
    // ignore: invalid_use_of_protected_member, invalid_use_of_visible_for_testing_member
    _gridViewDesynced |=
        _gridViewScrollController.position.activity is DragScrollActivity;
  }
  ...
  Future<void> _animateToPage(
    ...
    if (Settings.instance.autoloadAttachments &&
        (overrideRateLimit || !attachment.attachment.isRateLimited)) {
      _getController(attachment).loadFullAttachment().then((x) {
        if (mounted) {
          _currentAttachmentChanged.didUpdate();
        }
  ...
  double get _gridViewHeight {
    ...
    if ((widget.attachments.length + 1) >
        (maxRowCount * visibleSquaresPerRow)) {
      return maxHeight;
    }
    return squareSize *
        ((widget.attachments.length + 1) / (visibleSquaresPerRow)).ceil();
  }
...
Future<Attachment?> showGalleryPretagged({
  ...
  if (initialAttachment != null &&
      initialAttachment.attachment.shouldOpenExternally) {
  ...
  final keyboardOpen =
      MediaQueryData.fromView(View.of(context)).viewInsets.bottom > 100;
  if (!keyboardOpen) {
