...
Future<File> _moveFileOutOfDocumentsDir(File file) async {
  ...
  final docsResolved =
      await Persistence.documentsDirectory.resolveSymbolicLinks();
  if (parentResolved == docsResolved) {
    ...
    final parent = Persistence.temporaryDirectory
        .dir('inboxcache/${DateTime.now().millisecondsSinceEpoch}');
...
class ReplyBoxZone {
  ...
  final void Function(String text, {required PostIdentifier? backlink})
      onQuoteText;

  const ReplyBoxZone({required this.onTapPostId, required this.onQuoteText});
}

const _kNonWebmVideoExts = {
  'mp4',
  'mov',
  'm4v',
  'mkv',
...
class ReplyBoxState extends State<ReplyBox> {
  final _textFieldKey = GlobalKey<AdaptiveTextFieldState>(
      debugLabel: 'ReplyBoxState._textFieldKey');
  ...
  bool get showAttachmentOptions =>
      _showAttachmentOptions && !loading && _attachments.isNotEmpty;
  bool _show = false;
  ...
  bool get hasLoginSystem =>
      context.read<ImageboardSite>().loginSystem?.getSavedLoginFields() != null;
  ...
  String get defaultName =>
      context.read<Persistence?>()?.browserState.postingNames[widget.board] ??
      '';
  ImageboardBoardFlag? get defaultFlag =>
      context.read<Persistence?>()?.browserState.postingFlags[widget.board];
  ...
  Future<void> _setInitialAttachments(List<DraftPostFile> files) =>
      _attachmentsLock.protect(() async {
        ...
        final newFiles = files.where(
            (f) => !_attachments.any((a) => a.current.file.path == f.path));
            ...
            try {
              ...
              if (!file.existsSync()) {
                showToast(
                    ...
                    message:
                        'Previously-selected file is no longer accessible');
                continue;
              }
              final attachment = await _makeAttachment(null, file,
                  filenameWithoutExtension:
                      newFile.overrideFilenameWithoutExtension,
                  overrideRandomizeFilenames:
                      newFile.overrideRandomizeFilenames,
  ...
  Future<void> _addAttachment(File file) => _attachmentsLock.protect(() async {
        ...
        do {
          ...
          try {
            final attachment =
                await _makeAttachment(null, file, checkForDuplicateFile: true);
            if (!mounted || attachment == null) {
              return;
            }
            attachment.filenameController.addListener(() {
  ...
  void _onFilenameChanged(_ReplyBoxFile file) {
    if (file.filenameController.selection.isValid &&
        file.filenameController.text.isNotEmpty &&
        Settings.instance.randomizeFilenames &&
  ...
  Future<void> _scanForUrl() async {
    ...
    final rawUrl = linkify(text,
            linkifiers: const [LooseUrlLinkifier(fillInProtocol: true)],
            options:
    ...
    if (rawUrl != _lastFoundUrl && rawUrl != null) {
      try {
        ...
        final byteCount = response.headers
                .value(dio.Headers.contentLengthHeader)
                ?.tryParseInt ??
            0 /* chunked encoding? */;
        _proposedAttachmentUrl =
            (text: rawUrl, imageUrl: rawUrl, size: byteCount);
        if (mounted) setState(() {});
        return;
      } catch (e) {
  ...
  void _updateNeededPosts() {
    _neededPosts = _makeDraft().calculateNeededPosts(
        context.read<Imageboard>().persistence.getBoard(widget.board.s));
  }
  ...
  void initState() {
    ...
    _textFieldController =
        ReplyBoxTextEditingController(text: widget.initialDraft?.text);
    _textIsEmpty = text.isEmpty;
    _subjectFieldController =
        TextEditingController(text: widget.initialDraft?.subject);
    _optionsFieldController =
        TextEditingController(text: widget.initialDraft?.options);
    _nameFieldController = TextEditingController(
        text: persistence.browserState.postingNames[widget.board]);
    flag = widget.initialDraft?.flag ??
        persistence.browserState.postingFlags[widget.board];
    ...
    // owo.vg serves its own flag list, so flags must be reloaded whenever the
    // posting backend changes, not just when the board changes.
    Settings.instance.addListener(_onPostingBackendChanged);
    _loadBoardFlags();
    if (_nameFieldController.text.isNotEmpty ||
        _optionsFieldController.text.isNotEmpty ||
        (_disableLoginSystem && hasLoginSystem)) {
      _showOptions = true;
    }
  ...
  void _insertText(String insertedText,
      {bool addNewlineIfAtEnd = true, TextSelection? initialSelection}) {
    ...
    if (selection.isCollapsed) {
      ...
      _textFieldValue = TextEditingValue(
          ...
          text: _textFieldController.text.substring(0, currentPos) +
              insertedText +
              _textFieldController.text.substring(currentPos));
    } else {
      ...
      _textFieldValue = TextEditingValue(
          ...
          text: _textFieldController.text.substring(0, selection.baseOffset) +
              insertedText +
              _textFieldController.text.substring(selection.extentOffset));
    }
  }
  ...
  void showReplyBox() {
    ...
    for (final draft in Outbox.instance.queuedPostsFor(
        context.read<Imageboard>().key, widget.board, widget.threadId)) {
  ...
  Future<File?> _showTranscodeWindow({
    ...
    final solutions = [
      if (ext != transcode.outputFileExtension &&
          ...
          !(ext == 'jpg' && transcode.outputFileExtension == 'jpeg'))
        'to .${transcode.outputFileExtension}',
      if (size != null && maximumSize != null && (size > maximumSize))
        'compressing',
      if (durationInSeconds != null &&
          maximumDurationInSeconds != null &&
          (durationInSeconds > maximumDurationInSeconds))
        'clipping at ${maximumDurationInSeconds}s',
    ];
    if (width != null &&
        height != null &&
    ...
    if (switch ((codec, allowedCodecs[ext])) {
      (String codec_, Set<String> allowedCodecs_) =>
        !allowedCodecs_.contains(codec_),
      _ => false
    }) {
    ...
    try {
      ...
      Future.delayed(const Duration(milliseconds: 500), () {
        if (_attachmentProgress != null && mounted) {
          showToast(
              context: context,
              message: 'Converting$toastSuffix',
      ...
      if (toastedStart || showToastIfLong) {
        showToast(
            context: context,
            message: 'File converted${toastedStart ? '' : toastSuffix}',
  ...
  Future<_ReplyBoxFile?> _makeAttachment(
          PickedAttachment? originalAttachment, File newAttachment,
      ...
      _makeAttachmentLock.protect(() async {
        ...
        final randomizeChecksum =
            forceRandomizeChecksum || settings.randomizeChecksumOnUploadedFiles;
        setState(() {
        ...
        try {
          ...
          if (!file.uri.pathSegments.last.contains('.')) {
            ...
            file = await file.copy(Persistence.shareCacheDirectory
                .child('${file.uri.pathSegments.last}.${scan.guessExtension}'));
          }
            ...
            if (image != null && video != null) {
              ...
              file = await showAdaptiveDialog<File>(
                  ...
                  builder: (context) => AdaptiveAlertDialog(
                          ...
                          content: const Text(
                              'Which part of the Live Photo do you want to post?'),
                          actions: [
          ...
          if (scan.forceFormat == 'mjpeg') {
            ...
            file = await file.copy(Persistence.shareCacheDirectory
                .child('${file.uri.pathSegments.last}.jpeg'));
            ext = 'jpg';
            scan = await MediaScan.scan(file.uri);
          }
          // We may shrink under the size limit just by removing the audio
          if ((scan.duration, scan.audioBitrate)
              case (Duration duration, int audioBitrate)
              when !board.webmAudioAllowed) {
            final audioSize =
                (duration.inSecondsFloat * (audioBitrate / 8)).round();
          ...
          if (checkForDuplicateFile &&
              _attachments.any((a) =>
                  a.original.md5 == originalAttachment2.md5 &&
                  a.original.stat.size == originalAttachment2.stat.size)) {
            if (!mounted ||
                !await confirm(context, 'Add duplicate file?',
                    actionName: 'Add',
          ...
          if (ext == 'jpg' || ext == 'jpeg' || ext == 'webp' || ext == 'avif') {
            file = await _showTranscodeWindow(
                ...
                maximumDimension: forceMaximumDimension ??
                    settings.maximumImageUploadDimension,
                transcode: forceWebpToPng
                    ? MediaConversion.toPng(file.uri,
                        maximumSizeInBytes:
                            forceMaximumSizeInBytes ?? board.maxImageSizeBytes,
                        maximumDimension: forceMaximumDimension ??
                            settings.maximumImageUploadDimension,
                        removeMetadata: settings.removeMetadataOnUploadedFiles,
                        randomizeChecksum: randomizeChecksum)
                    : MediaConversion.toJpg(file.uri,
                        maximumSizeInBytes:
                            forceMaximumSizeInBytes ?? board.maxImageSizeBytes,
                        maximumDimension: forceMaximumDimension ??
                            settings.maximumImageUploadDimension,
            ...
            file = await _showTranscodeWindow(
                ...
                force: forceConvert,
                showToastIfLong: originalAttachment == null,
                transcode: forceGifToMp4
                    ? MediaConversion.toMp4(file.uri,
                        maximumSizeInBytes: forceMaximumSizeInBytes ??
                            board.maxWebmSizeBytes ??
                            board.maxImageSizeBytes,
                        maximumDimension: forceMaximumDimension ??
                            settings.maximumImageUploadDimension,
                        maximumDurationInSeconds:
                            board.maxWebmDurationSeconds?.toDouble(),
                        removeMetadata: settings.removeMetadataOnUploadedFiles,
                        randomizeChecksum: randomizeChecksum)
                    : MediaConversion.toGif(file.uri,
                        maximumSizeInBytes: forceMaximumSizeInBytes ??
                            board.maxWebmSizeBytes ??
                            board.maxImageSizeBytes,
                        maximumDimension: forceMaximumDimension ??
            ...
            file = await _showTranscodeWindow(
                ...
                size: size,
                maximumSize: forceMaximumSizeInBytes ??
                    board.maxWebmSizeBytes ??
                    board.maxImageSizeBytes,
                ...
                maximumDimension: forceMaximumDimension ??
                    settings.maximumImageUploadDimension,
                transcode: forceVideoToGif
                    ? MediaConversion.toGif(file.uri,
                        maximumSizeInBytes:
                            forceMaximumSizeInBytes ?? board.maxWebmSizeBytes,
                        maximumDimension: forceMaximumDimension ??
                            settings.maximumImageUploadDimension,
                        removeMetadata: settings.removeMetadataOnUploadedFiles,
                        randomizeChecksum: randomizeChecksum)
                    : MediaConversion.toWebm(file.uri,
                    ...
                    : (forceMp4ToWebm
                        ? MediaConversion.toWebm(file.uri,
                            ...
                            maximumDimension: forceMaximumDimension ??
                                settings.maximumImageUploadDimension,
                            removeMetadata:
                                settings.removeMetadataOnUploadedFiles,
                            randomizeChecksum: randomizeChecksum)
                        : MediaConversion.toMp4(file.uri,
                            stripAudio: !board.webmAudioAllowed,
                            maximumSizeInBytes: forceMaximumSizeInBytes ??
          ...
          if (file != null) {
            ...
            return _ReplyBoxFile(
                ...
                initialFilename:
                    filenameWithoutExtension ?? file.basenameWithoutExtension,
                spoiler: spoiler,
  ...
  Future<bool?> _shouldUseLoginSystem() async {
    ...
    bool? justOnce;
    Settings.autoLoginOnMobileNetworkSetting.value ??=
        await showAdaptiveDialog<bool>(
            context: context,
            builder: (context) => AdaptiveAlertDialog(
                    title: Text(
                        'Use ${site.loginSystem?.name} on mobile networks?'),
                    actions: [
                      AdaptiveDialogAction(
  ...
  Future<void> _shrinkFilesToFitMultiPosts(ImageboardBoard board) async {
    for (int i = 0; i < _attachments.length; i += board.filesPerPost) {
      ...
      for (int j = i; j < end; j++) {
        ...
        final thisLimit = ((ext == 'webm' || _kNonWebmVideoExts.contains(ext))
                ? board.maxWebmSizeBytes
                : board.maxImageSizeBytes) ??
            limit;
        if (thisLimit < limit) {
      ...
      for (int j = i; j < end; j++) {
        ...
        if (size > minQuota) {
          ...
          if (targetSize < size) {
            ...
            final currentExt =
                file.current.file.path.afterLast('.').toLowerCase();
            final originalExt =
                file.original.file.path.afterLast('.').toLowerCase();
            ...
            final newAttachment = await _makeAttachment(
                ...
                forceMaximumDimension:
                    (math.max(originalWidth, originalHeight) * quality).ceil(),
  ...
  Future<bool> _handleMultiFileResizing() async {
    ...
    final minimumPosts =
        math.max(1, (_attachments.length / board.filesPerPost).ceil());
    if (postsNeeded > minimumPosts) {
      final shrink = await showAdaptiveDialog<bool>(
          ...
          builder: (context) => AdaptiveAlertDialog(
                  ...
                  content: Text(
                      'Currently $postsNeeded posts would be needed to post the files as-is. Files could be shrunk to fit into ${describeCount(minimumPosts, 'post')}.'),
                  actions: [
                    AdaptiveDialogAction(
                        onPressed: () => Navigator.pop(context, true),
                        child: Text(
                            'Shrink (${describeCount(minimumPosts, 'post')})')),
                    AdaptiveDialogAction(
  ...
  void _setPendingOwoVgExtrasIfNeeded(Imageboard imageboard) {
    if (imageboard.site is Site4Chan &&
        Settings.instance.fourChanPostingBackend == OwoVgPostingBackend.owoVg) {
  ...
  Future<void> _submit() async {
          ...
          builder: (context) => AdaptiveAlertDialog(
                  ...
                  actions: [
                    AdaptiveDialogAction(
                        onPressed: () {
                          Clipboard.setData(
                              ClipboardData(text: _textFieldController.text));
                          showToast(
                              context: context,
                              message:
                                  'Copied "${_textFieldController.text}" to clipboard',
                    ...
                    AdaptiveDialogAction(
                        onPressed: () {
                          ...
                          final entry = Outbox.instance.submitPost(
                              imageboard.key, post, QueueStateIdle());
    ...
    final entry = Outbox.instance.submitPost(
        imageboard.key,
        post,
        QueueStateNeedsCaptcha(beforeModal: () {
          if (mounted && show) {
            autohid = true;
            hideReplyBox();
          }
  ...
  void _listenToReplyPosting(QueuedPost post) {
    ...
    void listener() async {
      ...
      } else if (state is QueueStateIdle<QueuedPost, PostReceipt> &&
          post == postingPost.value) {
  ...
  void _pickEmote() async {
    ...
    final pickedEmote = await Navigator.of(context).push<ImageboardEmote>(
        TransparentRoute(
  ...
  Widget _buildQueueButton() => Align(
      ...
      child: AnimatedSize(
          ...
          child: AnimatedBuilder(
              ...
              builder: (context, _) {
                final queue = Outbox.instance.queues[(
                  context.watch<Imageboard>().key,
                  widget.board,
                ...
                Widget build(BuildContext context) {
                  final ourCount = _submittingPosts.length +
                      (postingPost.value != null ? 1 : 0);
                  final activeCount = Outbox.instance.activeCount;
                  final othersCount = queue?.list
                          .where((e) => !e.state.isIdle && e.thread != thread)
                          .length ??
                      0;
                  final DateTime time;
                  ...
                  return Container(
                              ...
                              top: BorderSide(
                                  color: ChanceTheme
                                      .primaryColorWithBrightness20Of(
                                          context))),
                          color: ChanceTheme.primaryColorWithBrightness10Of(
                              context)),
                      child: AdaptiveButton(
                          onPressed: () async {
                            ...
                            if (selected != null) {
                              _onDraftTap(
                                  selected.post, selected.deleteOriginal);
                            }
                          },
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            if (time != now)
                              TimedRebuilder<String?>(
                                  ...
                                  builder: (context, str) {
                                    ...
                                    return Row(children: [
                                      ...
                                      Text(str,
                                          style:
                                              CommonTextStyles.tabularFigures)
                                    ]);
                                  }),
                            if (time != now && activeCount > ourCount)
                              const SizedBox(width: 16),
                            if (activeCount > ourCount) ...[
                              ...
                              Text([
                                describeCount(
                                    activeCount - ourCount, 'reply in outbox',
                                    plural: 'replies in outbox'),
                                if (othersCount > 0)
                                  '($othersCount queued on ${context.watch<ImageboardSite>().formatBoardName(widget.board.s)})'
                              ].join(' '))
                            ]
                          ])));
                }
                ...
                return AnimatedBuilder(
                    animation: queue, builder: (context, _) => build(context));
              })));
  ...
  Widget _buildSubmittingPosts() => AnimatedSize(
      ...
      child: show
          ? Column(
              mainAxisSize: MainAxisSize.min,
              children: _submittingPosts
                  .map((p) => QueueEntryWidget(
                        entry: p,
                        replyBoxMode: true,
          ...
          Expanded(
              child: Column(
                ...
                Row(children: [
                  Flexible(
                      child: Opacity(
                          opacity: settings.randomizeFilenames &&
                                  !file.overrideRandomizeFilenames
                              ? 0.5
                              : 1.0,
                          child: AdaptiveTextField(
                              ...
                              placeholder:
                                  file.current.file.basenameWithoutExtension,
                              maxLines: 1,
                              textCapitalization: TextCapitalization.none,
                              autocorrect: false,
                              enableIMEPersonalizedLearning:
                                  settings.enableIMEPersonalizedLearning,
                              smartDashesType: SmartDashesType.disabled,
                              smartQuotesType: SmartQuotesType.disabled,
                              keyboardAppearance:
                                  ChanceTheme.brightnessOf(context)))),
                  const SizedBox(width: 8),
                  Text('.${file.ext}'),
                  const SizedBox(width: 8),
                  AdaptiveIconButton(
                    ...
                    children: [
                      AdaptiveIconButton(
                          ...
                          icon: Row(mainAxisSize: MainAxisSize.min, children: [
                            Icon(settings.randomizeFilenames
                                ? (file.overrideRandomizeFilenames
                                    ? CupertinoIcons.minus_square
                                    : CupertinoIcons.checkmark_square)
                                : CupertinoIcons.square),
                            const Flexible(child: Text('Random filename'))
                          ]),
                          onPressed: () {
                            if (file.overrideRandomizeFilenames) {
                              ...
                              setState(() {
                                Settings.randomizeFilenamesSetting.value =
                                    !settings.randomizeFilenames;
                              });
                            }
                          }),
                      if (board.spoilers == true)
                        AdaptiveIconButton(
                            padding: EdgeInsets.zero,
                            minimumSize: Size.zero,
                            icon:
                      ...
                      AdaptiveThinButton(
                          ...
                          onPressed: () async {
                            ...
                            if (originalWidth == null ||
                                originalHeight == null) {
                              throw Exception(
                                  'Failed to get width and/or height');
                            }
                            double quality = 1.0;
                            final double qualityStep;
                            if (originalWidth > originalHeight) {
                              qualityStep = 2 / originalWidth;
                              if (file.current.scan.width
                                  case final newWidth?) {
                                quality = newWidth / originalWidth;
                              }
                            } else {
                              qualityStep = 2 / originalHeight;
                              if (file.current.scan.height
                                  case final newHeight?) {
                                quality = newHeight / originalHeight;
                              }
                            }
                            quality = math.min(quality,
                                1.0); // Floating point error or rounding or idk
                            final initialQuality = quality;
                            final currentExt = file.current.file.path
                                .afterLast('.')
                                .toLowerCase();
                            final originalExt = file.original.file.path
                                .afterLast('.')
                                .toLowerCase();
                            final offerJpg = originalExt == 'png';
                            final offerPng = originalExt == 'webp';
                            final offerWebm =
                                _kNonWebmVideoExts.contains(originalExt);
                            final offerGif = offerWebm || originalExt == 'webm';
                            final offerMp4 = originalExt == 'gif';
                            bool forcePngToJpg =
                                offerJpg && currentExt == 'jpg';
                            bool forceWebpToPng =
                                offerPng && currentExt == 'png';
                            bool forceMp4ToWebm =
                                offerWebm && currentExt == 'webm';
                            bool forceVideoToGif =
                                offerGif && currentExt == 'gif';
                            bool forceGifToMp4 =
                                offerMp4 && currentExt == 'mp4';
                            // Minimum 50px width, or 25% width, whichever is lower
                            final minQuality =
                                (25 * qualityStep).clamp(0, 0.25).toDouble();
                                            ...
                                            content: Column(
                                                ...
                                                children: [
                                                  ...
                                                  Row(children: [
                                                    Expanded(
                                                        child: Text(
                                                            '${(originalWidth * quality).roundToEven}x${(originalHeight * quality).roundToEven}')),
                                                    AdaptiveIconButton(
                                                        padding:
                                                            EdgeInsets.zero,
                                                        onPressed: quality <=
                                            ...
                                            actions: [
                                              ...
                                              if (offerGif)
                                                AdaptiveDialogAction(
                                                    ...
                                                    child: forceVideoToGif
                                                        ? Text(
                                                            'Restore to ${originalExt.toUpperCase()}')
                                                        : const Text(
                                                            'Convert to GIF')),
                                              if (offerPng)
                                                AdaptiveDialogAction(
                            ...
                            if (reencode ?? false) {
                              ...
                              try {
                                final newAttachment = await _makeAttachment(
                                    file.original, file.original.file,
                                    forceMaximumDimension: (math.max(
                          ...
                          child: Text(
                              [
                                if (file.ext == 'mp4' ||
                                    file.ext == 'webm') ...[
                                  if (file.current.scan.codec != null)
                                    file.current.scan.codec!.toUpperCase(),
                                  if (file.current.scan.hasAudio == true)
                                    'with audio'
                                  else
                                    'no audio',
                                  if (file.current.scan.duration != null)
                                    formatDuration(file.current.scan.duration!),
                                  if (file.current.scan.bitrate != null)
                                    '${(file.current.scan.bitrate! / (1024 * 1024)).toStringAsFixed(1)} Mbps',
                                ],
                                if (file.current.scan.width != null &&
                                    file.current.scan.height != null)
                                  '${file.current.scan.width}x${file.current.scan.height}',
                      ...
                      AdaptiveThinButton(
                          ...
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            const RotatedBox(
                                quarterTurns: 3,
                                child:
                                    Text('MD5', style: TextStyle(fontSize: 9))),
                            const SizedBox(width: 2),
                            Text(file.current.md5.substring(0, 6).toLowerCase(),
                          ...
                          onPressed: () async {
                            ...
                            try {
                              final newAttachment = await _makeAttachment(
                                  file.original, file.current.file,
                                  forceRandomizeChecksum: true,
                ...
                minWidth: 100 *
                    math.min(
                        ...
                        _attachments.fold(
                            ...
                            (ratio, file) => math.max(
                                ratio,
                                (file.current.scan.width ?? 0) /
                                    (file.current.scan.height ?? 1)))),
              ),
                  ...
                  child: Hero(
                      ...
                      createRectTween: (startRect, endRect) {
                        ...
                        return CurvedRectTween(
                            curve: Curves.ease, begin: startRect, end: endRect);
                      },
                      child: MediaThumbnail(
                          uri: file.current.file.uri, fit: BoxFit.contain)),
                  onTap: () async {
    ...
    return Container(
        decoration: BoxDecoration(
            border: Border(
                top: BorderSide(
                    color:
                        ChanceTheme.primaryColorWithBrightness20Of(context))),
            color: ChanceTheme.backgroundColorOf(context)),
        padding: const EdgeInsets.only(top: 9, left: 8, right: 8, bottom: 10),
        child: Row(children: [
          Flexible(
              child: AdaptiveTextField(
                  ...
                  suffix: AdaptiveIconButton(
                      ...
                      minimumSize: Size.zero,
                      onPressed: _previouslyUsedNames.isEmpty
                          ? null
                          : () async {
                              final choice = await showAdaptiveModalPopup<String>(
                                  ...
                                  builder: (context) => AdaptiveActionSheet(
                                      title:
                                          const Text('Previously-used names'),
                                      actions: _previouslyUsedNames
                                          .map((name) =>
          ...
          Flexible(
              child: AdaptiveTextField(
                  ...
                  suffix: AdaptiveIconButton(
                      padding: const EdgeInsets.only(right: 8),
                      minimumSize: Size.zero,
                      onPressed: _previouslyUsedOptions.isEmpty
                          ? null
                          : () async {
                              final choice = await showAdaptiveModalPopup<String>(
                                  context: context,
                                  builder: (context) => AdaptiveActionSheet(
                                      title:
                                          const Text('Previously-used options'),
                                      actions: _previouslyUsedOptions
                                          .map((name) =>
                                              AdaptiveActionSheetAction(
                                                  onPressed: () =>
                                                      Navigator.pop(
          ...
          if (site.supportsWebPostingFallback)
            Padding(
                padding: const EdgeInsets.only(left: 8),
                child: AdaptiveThinButton(
                    padding: const EdgeInsets.all(4),
                    onPressed: (_attachmentProgress != null ||
                            (!kDebugMode &&
                                _textIsEmpty &&
                        ...
                        : () async {
                            ...
                            final board =
                                imageboard.persistence.getBoard(widget.board.s);
                            final (currentFiles, nextFiles) =
                                draft.splitFiles(board);
                            ...
                            final receipt = await Navigator.of(context,
                                    rootNavigator: true)
                                .push<PostReceipt>(adaptivePageRoute(
                                    useFullWidthGestures:
                                        false, // Some captchas have sliding thing
                                    builder: (context) => CookieBrowser(
                                        initialUrl: Uri.parse(site.getWebUrl(
                                            board: widget.board.s,
                                            threadId: widget.threadId)),
                            ...
                            if (receipt != null) {
                              ...
                              if (nextFiles.isNotEmpty) {
                                ...
                                if (widget.threadId != null) {
                                  ...
                                  final threadState = imageboard.persistence
                                      .getThreadState(ThreadIdentifier(
                                          widget.board.s, receipt.id));
                              ...
                              _rootFocusNode.unfocus();
                              if (Settings
                                  .instance.closeReplyBoxAfterSubmitting) {
                                hideReplyBox();
                              }
                            }
                          },
  ...
  Widget _buildProposedAttachment(BuildContext context) => Container(
      ...
      height: 64,
      child: _proposedAttachmentUrl == null
          ? const SizedBox()
          : Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
              if (_proposedAttachmentUrl != null)
                Padding(
                    padding: const EdgeInsets.all(8),
                    child: _proposedAttachmentUrl!.size > 4e6 /* 4 MB */
                        ? const SizedBox(
                      ...
                      child: Text.rich(
                          TextSpan(children: [
                            ...
                            TextSpan(
                                ...
                                style: TextStyle(
                                    color:
                                        ChanceTheme.backgroundColorOf(context)
                        ...
                        try {
                          if (proposed.size > 4e6 /* 4 MB */) {
                            ...
                            final ok = await confirm(context,
                                'Really download this ${formatFilesize(proposed.size)} file?');
                          ...
                          if (proposed.text.beforeFirst('?') ==
                              proposed.imageUrl.beforeFirst('?')) {
                            final original = _textFieldController.text;
                            final replaced =
                                original.replaceFirst(proposed.text, '');
                            if (replaced.length !=
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              if (widget.threadId == null) ...[
                AdaptiveTextField(
                    ...
                    spellCheckConfiguration: !settings.enableSpellCheck ||
                            (isOnMac && isDevelopmentBuild)
                        ? null
              ...
              if (subjectCharacterLimit != null ||
                  board.maxCommentCharacters != null)
                AnimatedBuilder(
                    animation: Listenable.merge(
                        [_textFieldController, _subjectFieldController]),
                    builder: (context, _) {
                      final greyColor =
                          ChanceTheme.primaryColorWithBrightness50Of(context);
                      final subjectLimit =
                          subjectCharacterLimit ?? infiniteLimit;
                      final subjectLength = _subjectFieldController.text.length;
                      final showSubjectLimit =
                          subjectLength > (subjectLimit * 0.5);
                      final subjectLimitColor =
                          subjectLength > subjectLimit ? Colors.red : greyColor;
                      final textLimit =
                          board.maxCommentCharacters ?? infiniteLimit;
                      final textLength = _textFieldController.text.length;
                      final showTextLimit = textLength > (textLimit * 0.5);
                      final textLimitColor =
                          textLength > textLimit ? Colors.red : greyColor;
                      return IgnorePointer(
                          child: AnimatedSize(
                              alignment: Alignment.topCenter,
                              duration: const Duration(milliseconds: 250),
                              curve: Curves.ease,
                              child: (showSubjectLimit || showTextLimit)
                                  ? Padding(
                  ...
                  child: ConstrainedBox(
                      ...
                      child: IntrinsicHeight(
                          child: WidgetDecoration(
                              ...
                              child: Padding(
                                  padding: settings.materialStyle
                                  ...
                                  child: AdaptiveTextField(
                                    ...
                                    contentInsertionConfiguration:
                                      ...
                                      try {
                                        ...
                                        String filename = Uri.parse(content.uri)
                                            .pathSegments
                                            .last;
                                        if (!filename.contains('.')) {
                                          filename +=
                                              '.${content.mimeType.afterLast('/')}';
                                        }
                                        final f = Persistence
                                            .shareCacheDirectory
                                            .file(
                                                                      ...
                                                                      if (time !=
                                                                        ...
                                                                        GreedySizeCachingBox(
                                                                            ...
                                                                            child: TimedRebuilder(
                                                                                ...
                                                                                builder: (context, delta) => Text(' ($delta)', style: CommonTextStyles.tabularFigures)))
                                                                    ]));
                                                          });
                                                    });
                                              }),
  ...
  Widget _buildButtons(BuildContext context) {
    ...
    final snippets =
        context.read<ImageboardSite>().getBoardSnippets(widget.board.s);
    final defaultTextStyle = DefaultTextStyle.of(context).style;
    final settings = context.watch<Settings>();
    final isSite4Chan = imageboard.site is Site4Chan;
    final useOwoVgPosting =
        isSite4Chan &&
        settings.fourChanPostingBackend == OwoVgPostingBackend.owoVg;
          ...
          child: ListView(
              ...
              children: [
                  ...
                  GestureDetector(
                      onLongPress: loading
                          ...
                          : () {
                              ...
                              _insertText(
                                  snippet.wrap(_textFieldController.selection
                                      .textInside(_textFieldController.text)),
                                  addNewlineIfAtEnd: false,
                                  initialSelection:
                      ...
                      child: AdaptiveIconButton(
                          onPressed: loading
                              ...
                              : () async {
                                  ...
                                  final controller =
                                      _snippetControllers.putIfAbsent(snippet,
                                          () => TextEditingController());
                                  if (!initialSelection.isCollapsed) {
                                    controller.text = initialSelection
                                        .textInside(_textFieldController.text);
                                  }
                                  final content = await showAdaptiveDialog<
                                          String>(
                                      context: context,
                                      barrierDismissible: true,
                                      builder: (context) => AdaptiveAlertDialog(
                                              title:
                                                  Text('${snippet.name} block'),
                                              content: Padding(
                                                  padding:
                                                            ...
                                                            builder: (context) =>
                                                                AdaptiveAlertDialog(
                                                                    ...
                                                                    content: ChangeNotifierProvider<
                                                                        ...
                                                                        builder: (context, _) => DefaultTextStyle(style: defaultTextStyle, child: Text.rich(snippet.previewBuilder!(controller.text).build(context, post, context.watch<PostSpanZoneData>(), context.watch<Settings>(), context.watch<SavedTheme>(), const PostSpanRenderOptions())))),
                                                                    actions: [
                                                                      AdaptiveDialogAction(
                                                                          isDefaultAction:
                                                                              true,
                                  ...
                                  if (content != null) {
                                    _insertText(snippet.wrap(content),
                                        addNewlineIfAtEnd: false,
                ...
                AnimatedSize(
                    ...
                    duration: const Duration(milliseconds: 250),
                    curve: Curves.ease,
                    child: _attachmentProgress != null
                        ? Row(mainAxisSize: MainAxisSize.min, children: [
                            ...
                            SizedBox(
                                ...
                                child: AdaptiveButton(
                                    padding: EdgeInsets.zero,
                                    onPressed: _attachmentProgress?.$2 == null
                                        ? null
                                        : () async {
                                            final confirmed = await confirm(
                                                context, 'Stop conversion?',
                                                actionName: 'Stop');
                                            if (confirmed) {
                                    ...
                                    child: ClipRRect(
                                        ...
                                        child: ValueListenableBuilder<double?>(
                                            valueListenable:
                                                _attachmentProgress!.$2?.progress ??
                                                    const StoppedValueListenable(
                                                        null),
                            ...
                            if (board.filesPerPost > 0)
                              AnimatedBuilder(
                                  ...
                                  builder: (context, _) => Row(
                                          ...
                                          children: [
                                            for (final file
                                                in receivedFilePaths.reversed)
                                              if (!_attachments.any((a) =>
                                                  a.original.file.path == file))
                                                GestureDetector(
                                                    onLongPress: loading
                                                        ? null
                                                        : () async {
                            ...
                            if (_attachments.isNotEmpty)
                              Container(
                                  ...
                                  color: settings.theme
                                      .primaryColorWithBrightness(0.2)),
                            if (_attachments.isNotEmpty)
                              AdaptiveIconButton(
                                  padding:
                                      const EdgeInsets.only(left: 8, right: 8),
                                  onPressed:
                                      loading ? null : expandAttachmentOptions,
                                  icon: Row(
                                      mainAxisSize: MainAxisSize.min,
                                      children: [
      ...
      GestureDetector(
          onLongPress: loading
              ...
              : () {
                  ...
                  final entry = Outbox.instance
                      .submitPost(imageboard.key, post, QueueStateIdle());
                  _submittingPosts.add(entry);
                  _listenToReplyPosting(entry);
                  draft = null; // Clear
                  widget.onDraftChanged(null);
          ...
          child: Opacity(
              ...
              child: AdaptiveIconButton(
                  onPressed: _attachmentProgress != null
                      ...
                      : (loading
                          ? _cancel
                          : switch ((kDebugMode, _textIsEmpty, _attachments)) {
  ...
  void _onDraftTap(QueuedPost entry, bool deleteOriginal) {
    ...
    if (_isNonTrivial(old) && old != entry.post) {
      ...
      Outbox.instance
          .submitPost(context.read<Imageboard>().key, old, QueueStateIdle());
    }
    setState(() {});
  }
    ...
    return Focus(
        ...
        child: TransformedMediaQuery(
            transformation: (context, mq) =>
                mq.removePadding(removeTop: true, removeBottom: true),
            child: PrototypeLayoutWidget(
                prototype: Column(mainAxisSize: MainAxisSize.min, children: [
                  _buildQueueButton(),
                  Expander(
                ...
                child: MaybeScrollbar(
                    child: CustomScrollView(
                        ...
                        slivers: [
                      SliverToBoxAdapter(child: _buildQueueButton()),
                      SliverToBoxAdapter(
                          child: Expander(
                              expanded: _showSubmittingPosts,
                              bottomSafe: true,
                              ...
                              child: Focus(
                                  ...
                                  child: ReorderableListView(
                                      ...
                                      onReorder: (oldIndex, newIndex) {
                                        ...
                                        final item =
                                            _attachments.removeAt(oldIndex);
                          ...
                          child: Expander(
                              ...
                              child: GestureDetector(
                                  ...
                                  onVerticalDragStart: (event) {
                                    _replyBoxHeightOffsetAtPanStart =
                                        settings.replyBoxHeightOffset;
                                  ...
                                  onVerticalDragUpdate: (event) {
                                    ...
                                    setState(() {
                                      _willHideOnPanEnd =
                                          ((view.physicalSize.height / r) -
                                                  event.globalPosition.dy) <
                                              (view.viewInsets.bottom / r);
...
class ReplyBoxTextEditingController extends TextEditingController {
  ReplyBoxTextEditingController({super.text});
...
enum _ReplyBoxLayoutId { body, replyBox }
...
class _ReplyBoxLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  _ReplyBoxLayoutDelegate(
      {required this.topPadding, required this.bottomPadding});

  @override
  void performLayout(Size size) {
    final replyBoxSize = layoutChild(
        _ReplyBoxLayoutId.replyBox,
        BoxConstraints(
            minWidth: size.width,
            maxWidth: size.width,
    ...
    positionChild(
        _ReplyBoxLayoutId.replyBox, Offset(0, threadHeight - bottomPadding));
    layoutChild(_ReplyBoxLayoutId.body,
        BoxConstraints.tightFor(width: size.width, height: threadHeight));
...
class ReplyBoxLayout extends StatelessWidget {
  ...
  const ReplyBoxLayout({required this.body, required this.replyBox, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return CustomMultiChildLayout(
        ...
        children: [
          LayoutId(id: _ReplyBoxLayoutId.body, child: body),
          LayoutId(id: _ReplyBoxLayoutId.replyBox, child: replyBox)
        ]);
  }
}
