...
class VideoControls extends StatefulWidget {
  ...
  const VideoControls(
      {required this.controller, this.showMuteButton = true, Key? key})
      : super(key: key);
    ...
    return SizedBox(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
          ...
          ValueListenableBuilder(
              ...
              builder: (context, Duration positionValue, _) => SizedBox(
                  ...
                  child: FittedBox(
                      ...
                      child: Text(formatDuration(positionValue),
                  ...
                  child: GestureDetector(
                      onTapUp: (x) async {
                        await widget.controller
                            .onLongPressStart(absolute: true);
                        await widget.controller.onCoalescedLongPressUpdate(
                            x.localPosition.dx / _calculateSliderWidth());
                        await widget.controller.onLongPressEnd();
                      },
                      onHorizontalDragStart: (x) =>
                          widget.controller.onLongPressStart(absolute: true),
                      onHorizontalDragUpdate: (x) => widget.controller
                          .onLongPressUpdate(
                              x.localPosition.dx / _calculateSliderWidth()),
                              ...
                              children: [
                                ValueListenableBuilder(
                                    valueListenable: widget
                                        .controller.showLoadingProgress,
                                    builder: (context, showLoadingProgress, _) => (showLoadingProgress ||
                                            !widget.controller.cacheCompleted)
                                        ? ValueListenableBuilder(
                                ...
                                ValueListenableBuilder(
                                    valueListenable: position,
                                    builder: (context, Duration positionValue,
                                            _) =>
                                        LinearProgressIndicator(
                                            minHeight: 44,
                                            value: switch (value
                                                ?.duration.inMilliseconds) {
                                              int ms => positionValue
                                                      .inMilliseconds /
                                                  ms.clamp(1, double.maxFinite),
                                              null => 0
                                            },
          ...
          SizedBox(
              ...
              child: FittedBox(
                  fit: BoxFit.scaleDown,
                  child: Text(formatDuration(value?.duration),
                      style: TextStyle(color: primaryColor)))),
          if (widget.controller.hasAudio && widget.showMuteButton)
            AnimatedBuilder(
                animation: Settings.instance.muteAudio,
                builder: (context, _) {
                  final mutedNow =
                      value?.volume == 0 || Settings.instance.muteAudio.value;
          ...
          AdaptiveIconButton(
            icon: Icon((widget.controller.currentlyWithinLongPress
                    ? widget.controller.playingBeforeLongPress
                    : (value?.playing ?? false))
